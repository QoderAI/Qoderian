...
describe('VaultMentionIndex', () => {
  ...
  it('returns cached vault files and folders without reloading until dirty', () => {
    ...
    const index = new VaultMentionIndex(app);

    expect(index.getCachedVaultFiles()).toEqual(files);
    expect(index.getCachedVaultFiles()).toEqual(files);
    expect(index.getCachedVaultFolders()).toEqual([{ name: 'notes', path: 'notes' }]);
    expect(index.getCachedVaultFolders()).toEqual([{ name: 'notes', path: 'notes' }]);

    expect(app.vault.getFiles).toHaveBeenCalledTimes(1);
    expect(app.vault.getAllLoadedFiles).toHaveBeenCalledTimes(1);

    index.markFilesDirty();
    index.markFoldersDirty();
    index.getCachedVaultFiles();
    index.getCachedVaultFolders();
  ...
  it('reports file load errors only once while continuing to return an empty result', () => {
    ...
    const index = new VaultMentionIndex(app, { onFileLoadError });

    expect(index.getCachedVaultFiles()).toEqual([]);
    expect(index.getCachedVaultFiles()).toEqual([]);
