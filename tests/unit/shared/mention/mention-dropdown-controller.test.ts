...
} from '@/shared/mention/mention-dropdown-controller';
import type { MentionExtensionProvider } from '@/shared/mention/types';
...
jest.mock('@/shared/components/selectable-dropdown', () => ({
...
function getLatestDropdownRenderOptions(): any {
  const { SelectableDropdown } = jest.requireMock('@/shared/components/selectable-dropdown');
...
function createMockExtensionProvider(): MentionExtensionProvider {
  return {
    getItems: jest.fn((searchText: string) => ({
      exclusive: searchText.toLowerCase().startsWith('agents/'),
      items: [{
        type: 'extension',
        key: 'agent:explore',
        displayText: '@Explore',
        replacement: '@Explore (agent) ',
        renderIcon: jest.fn(),
      }],
    })),
  };
...
describe('MentionDropdownController', () => {
  ...
  beforeEach(() => {
    ...
    const { externalContextScanner } = jest.requireMock('@/core/context/external-context-scanner');
  ...
  describe('extension provider', () => {
    it('queries and renders injected extension items', () => {
      const provider = createMockExtensionProvider();
      controller.setExtensionProvider(provider);
      ...
      expect(provider.getItems).toHaveBeenCalledWith('Agents/exp');
      expect(getLatestDropdownRenderOptions().items[0]).toMatchObject({
        type: 'extension',
        key: 'agent:explore',
      });
    });

    it('hides an open dropdown when the provider changes', () => {
      controller.setExtensionProvider(createMockExtensionProvider());
      inputEl.value = '@';
  ...
  describe('handleInputChange', () => {
    ...
    it('hides external context dropdown after completed spaced mention stops matching', async () => {
      const { externalContextScanner } = jest.requireMock('@/core/context/external-context-scanner');
      (externalContextScanner.scanPaths as jest.Mock).mockReturnValue([
        {
    ...
    it('handles @ at start of line', () => {
      (callbacks.getCachedVaultFiles as jest.Mock).mockReturnValue([{
        path: 'Explore.md', name: 'Explore.md', stat: { mtime: 1 },
      }]);
  ...
  describe('handleKeydown', () => {
    ...
    it('does not intercept IME candidate navigation', () => {
      controller.setExtensionProvider(createMockExtensionProvider());
      inputEl.value = '@';
      inputEl.selectionStart = 1;
      controller.handleInputChange();
    ...
    it('cannot select stale results while a debounced refresh is pending', () => {
      const onSelect = jest.fn();
      controller.setExtensionProvider({
        getItems: () => ({
          items: [{
            type: 'extension', key: 'old', displayText: '@old',
            replacement: '@old ', renderIcon: jest.fn(), onSelect,
          }],
        }),
      });
      inputEl.value = '@';
      ...
      controller.handleInputChange();
      const event = { key: 'Enter', isComposing: false, preventDefault: jest.fn() } as any;
  ...
  describe('extension selection callback', () => {
    it('inserts replacement and calls the extension callback', () => {
      const onSelect = jest.fn();
      const testCallbacks = createMockCallbacks();
      ...
      testController.setExtensionProvider({
        getItems: () => ({
          items: [{
            type: 'extension',
            key: 'custom-agent',
            displayText: '@custom-agent',
            replacement: '@custom-agent (agent) ',
            renderIcon: jest.fn(),
            onSelect,
          }],
        }),
      ...
      testInput.value = '@custom';
      testInput.selectionStart = 7;
      ...
      expect(testInput.value).toBe('@custom-agent (agent) ');
  ...
  describe('input debouncing', () => {
    it('debounces rapid input changes', () => {
      ...
      expect(provider.getItems).toHaveBeenCalledTimes(1);
    });
    ...
    it('processes input after debounce delay', () => {
      ...
      expect(provider.getItems).toHaveBeenCalled();
    });
  });
  ...
  describe('vault folder mentions', () => {
    ...
    it('defaults selection to first vault item when special items exist', () => {
      ...
      const localInput = createMockInput();
      const localController = new MentionDropdownController(createMockEl(), localInput, localCallbacks);
      localController.setExtensionProvider({
        getItems: () => ({
          items: [{
            type: 'extension',
            key: 'filesystem',
    ...
    it('renders vault folder text in @path/ format', () => {
      ...
      const nameEl = itemEl.querySelector('.qoderian-mention-name-folder');
      expect(nameEl?.textContent).toBe('@src/');

      localController.destroy();
    });

    it('still shows vault folder matches when slash search overlaps external context', async () => {
      const { externalContextScanner } = jest.requireMock('@/core/context/external-context-scanner');
      (externalContextScanner.scanPaths as jest.Mock).mockReturnValue([]);
