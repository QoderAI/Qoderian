...
import { normalizePathForVault } from '@/core/fs/path';
import { buildInlineEditInputDecorations, normalizeInsertionText } from '@/features/inline-edit/ui/modal';
...
function diffToHtml(ops: DiffOp[]): string {
  return ops
    .map((op) => {
      ...
      switch (op.type) {
        case 'delete':
          return `<span class="qoderian-diff-del">${escaped}</span>`;
        case 'insert':
          return `<span class="qoderian-diff-ins">${escaped}</span>`;
        default:
          return escaped;
      }
    })
...
describe('InlineEditModal - Word-level Diff', () => {
  ...
  describe('diffToHtml', () => {
    ...
    it('should wrap deleted text with del class', () => {
      ...
      expect(html).toContain('qoderian-diff-del');
    ...
    it('should wrap inserted text with ins class', () => {
      ...
      expect(html).toContain('qoderian-diff-ins');
