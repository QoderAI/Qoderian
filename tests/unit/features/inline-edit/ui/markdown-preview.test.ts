...
import { renderInlineEditMarkdownPreview } from '@/features/inline-edit/ui/markdown-preview';
...
describe('renderInlineEditMarkdownPreview', () => {
  ...
  it('normalizes LaTeX backslash delimiters for Obsidian', async () => {
    const app = { vault: {}, metadataCache: {} } as any;
    const component = {} as any;
    const container = createMockEl();
  ...
  it('falls back to raw markdown when Obsidian rendering fails', async () => {
    ...
    expect(container.children[0].hasClass('qoderian-inline-markdown-fallback')).toBe(true);
