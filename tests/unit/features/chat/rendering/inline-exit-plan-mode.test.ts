...
import { InlineExitPlanMode } from '@/features/chat/rendering/inline-exit-plan-mode';
...
function findRoot(container: any): any {
  return container.querySelector('.qoderian-plan-approval-inline');
}

function findItems(root: any): any[] {
  return root.querySelectorAll('qoderian-ask-item');
}

describe('InlineExitPlanMode', () => {
  it('resolves with approve-new-session and includes plan content when readable', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'qoderian-'));
    const plansDir = path.join(tmpDir, '.qoder', 'plans');
    ...
    expect(container.querySelector('.qoderian-plan-permissions-list')).toBeTruthy();
  ...
  it('shows a read error when plan file cannot be read', () => {
    ...
    const widget = new InlineExitPlanMode(
      container,
      { planFilePath: '/path/.qoder/plans/does-not-exist.md' },
      resolve,
      undefined,
      undefined,
      '/.qoder/plans/',
    ...
    const root = findRoot(container);
    expect(root).toBeTruthy();
    expect(container.querySelector('.qoderian-plan-read-error')).toBeTruthy();
  ...
  it('rejects plan file paths outside .qoder/plans/', () => {
    const container = createMockEl();
    const resolve = jest.fn();
  ...
  it('supports feedback flow and Escape when input is focused', () => {
    ...
    const feedbackInput = feedbackRow.querySelector('qoderian-ask-custom-text');
