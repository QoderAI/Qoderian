...
import { renderDiffContent, splitIntoHunks } from '@/features/chat/rendering/diff-renderer';
import { countLineChanges, structuredPatchToDiffLines } from '@/qoder/tools/diff';
...
describe('DiffRenderer', () => {
  ...
  describe('renderDiffContent', () => {
    it('should render all lines when all-inserts count is within cap', () => {
      ...
      expect(countByClass(container, 'qoderian-diff-insert')).toBe(20);
      expect(countByClass(container, 'qoderian-diff-separator')).toBe(0);
    });
    ...
    it('should cap all-inserts diff at 20 lines with remainder message', () => {
      ...
      const separator = container._children.find(
        (c: any) => c.hasClass('qoderian-diff-separator'),
      );
    ...
    it('should not cap mixed diff lines (edits with context)', () => {
      ...
      expect(countByClass(container, 'qoderian-diff-insert')).toBe(30);
    });
  });
});
