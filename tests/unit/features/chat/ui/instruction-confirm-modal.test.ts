...
} from '@/features/chat/ui/instruction-confirm-modal';
...
function clickButton(root: any, text: string): void {
  const buttons = findAllByClass(root, 'qoderian-instruction-btn');
...
describe('InstructionModal', () => {
  describe('onOpen', () => {
    it('renders the raw instruction text', () => {
      ...
      const originalEl = findByClass(contentEl, 'qoderian-instruction-original');
    ...
    it('starts in loading state', () => {
      ...
      const loadingEl = findByClass(contentEl, 'qoderian-instruction-loading');
      expect(loadingEl).not.toBeNull();
      expect(loadingEl.hasClass('qoderian-hidden')).toBe(false);

      const clarificationEl = findByClass(contentEl, 'qoderian-instruction-clarification-section');
      expect(clarificationEl.hasClass('qoderian-hidden')).toBe(true);

      const confirmationEl = findByClass(contentEl, 'qoderian-instruction-confirmation-section');
      expect(confirmationEl.hasClass('qoderian-hidden')).toBe(true);
    });
    ...
    it('renders Cancel button in loading state', () => {
      ...
      const buttons = findAllByClass(contentEl, 'qoderian-instruction-btn');
  ...
  describe('showClarification', () => {
    it('transitions to clarification state', () => {
      ...
      const loadingEl = findByClass(contentEl, 'qoderian-instruction-loading');
      expect(loadingEl.hasClass('qoderian-hidden')).toBe(true);

      const clarificationEl = findByClass(contentEl, 'qoderian-instruction-clarification-section');
      expect(clarificationEl.hasClass('qoderian-hidden')).toBe(false);
    });
    ...
    it('displays the clarification text', () => {
      ...
      const clarificationTextEl = findByClass(contentEl, 'qoderian-instruction-clarification');
  ...
  describe('showConfirmation', () => {
    it('transitions to confirmation state', () => {
      ...
      const confirmationEl = findByClass(contentEl, 'qoderian-instruction-confirmation-section');
      expect(confirmationEl.hasClass('qoderian-hidden')).toBe(false);
    });
    ...
    it('displays the refined instruction', () => {
      ...
      const refinedEl = findByClass(contentEl, 'qoderian-instruction-refined');
