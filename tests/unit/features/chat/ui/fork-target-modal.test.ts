...
import { chooseForkTarget } from '@/features/chat/ui/fork-target-modal';
...
function getOptionItems(): Array<{ text: string; click: () => void }> {
  const listEl = lastModalInstance.contentEl.children?.find(
    (c: any) => c.hasClass?.('qoderian-fork-target-list'),
  );
  if (!listEl) return [];
  return (listEl.children || [])
    .filter((c: any) => c.hasClass?.('qoderian-fork-target-option'))
    .map((c: any) => ({
      text: c.textContent,
      click: () => {
