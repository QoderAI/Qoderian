import { InstructionModeManager } from '@/features/chat/ui/instruction-mode-manager';
...
describe('InstructionModeManager', () => {
  it('should enter instruction mode on # keystroke when input is empty', () => {
    ...
    expect(wrapper.addClass).toHaveBeenCalledWith('qoderian-input-instruction-mode');
  });
  ...
  it('should exit instruction mode when input is cleared', () => {
    ...
    expect(wrapper.removeClass).toHaveBeenCalledWith('qoderian-input-instruction-mode');
  });
