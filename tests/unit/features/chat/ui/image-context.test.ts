...
function createContainerWithInputWrapper(): { container: any; inputWrapper: any } {
  ...
  const inputWrapper = container.createDiv({ cls: 'qoderian-input-wrapper' });
...
describe('ImageContextManager - Private Helpers', () => {
  ...
  describe('Image preview rendering', () => {
    ...
    it('renderImagePreview should create chip with thumbnail, info, and remove button', () => {
      ...
      expect(chipEl.hasClass('qoderian-image-chip')).toBe(true);

      const thumbEl = chipEl.querySelector('.qoderian-image-thumb');
      expect(thumbEl).not.toBeNull();

      const infoEl = chipEl.querySelector('.qoderian-image-info');
      expect(infoEl).not.toBeNull();

      const removeEl = chipEl.querySelector('.qoderian-image-remove');
    ...
    it('remove button should delete the image and update preview', () => {
      ...
      const removeEl = firstChip.querySelector('.qoderian-image-remove');
  ...
  describe('showFullImage', () => {
    ...
    it('should create modal overlay with image', () => {
      ...
      expect(mockBody.createDiv).toHaveBeenCalledWith({ cls: 'qoderian-image-modal-overlay' });
    });
