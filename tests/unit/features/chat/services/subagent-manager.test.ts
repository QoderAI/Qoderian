...
import { SubagentManager } from '@/features/chat/services/subagent-manager';
import { QoderTaskResultInterpreter } from '@/qoder/runtime/qoder-task-result-interpreter';
import { createStopSubagentHook } from '@/qoder/runtime/subagent-hooks';

jest.mock('@/features/chat/rendering/subagent-renderer', () => ({
...
const createManager = () => {
  ...
  const manager = new SubagentManager(
    ...
    new QoderTaskResultInterpreter(),
  );
...
${inlineOutput}
    ...
    it('ignores truncated full output files outside trusted temp roots', () => {
      ...
      const untrustedDir = mkdtempSync(join(homeDir, '.qoderian-untrusted-'));
...
Only this is the final result.
  ...
  describe('renderPendingTask', () => {
    ...
    it('does not increment spawned counter when rendering throws', () => {
      const { createSubagentBlock } = jest.requireMock('@/features/chat/rendering/subagent-renderer');
  ...
  describe('sync subagent operations', () => {
    ...
    it('adds tool call to sync subagent', () => {
      const { addSubagentToolCall } = jest.requireMock('@/features/chat/rendering/subagent-renderer');
      const { manager } = createManager();
      const parentEl = createMockEl();
    ...
    it('updates tool result in sync subagent', () => {
      const { updateSubagentToolResult } = jest.requireMock('@/features/chat/rendering/subagent-renderer');
      const { manager } = createManager();
      const parentEl = createMockEl();
    ...
    it('finalizes sync subagent and removes from map', () => {
      const { finalizeSubagentBlock } = jest.requireMock('@/features/chat/rendering/subagent-renderer');
      const { manager } = createManager();
      const parentEl = createMockEl();
