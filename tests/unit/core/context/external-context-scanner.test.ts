...
describe('externalContextScanner', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'qoderian-test-'));
  ...
  describe('scanPaths', () => {
    ...
    it('should handle non-existent paths', async () => {
      const files = await externalContextScanner.scanPaths(['/non/existent/path']);

      expect(files).toEqual([]);
    });

    it('should handle multiple external context paths', async () => {
      // Create a second temp directory
      const tempDir2 = fs.mkdtempSync(path.join(os.tmpdir(), 'qoderian-test2-'));
      fs.writeFileSync(path.join(tempDir2, 'file4.js'), 'content4');

      try {
        const files = await externalContextScanner.scanPaths([tempDir, tempDir2]);
    ...
    it('strictly caps the number of files returned for one context path', async () => {
      for (let index = 0; index < 1005; index++) {
        fs.writeFileSync(path.join(tempDir, `limit-${index}.txt`), 'content');
      }
  ...
  describe('caching', () => {
    it('should cache results', async () => {
      // First scan
      const files1 = await externalContextScanner.scanPaths([tempDir]);

      // Add a new file
      fs.writeFileSync(path.join(tempDir, 'new-file.txt'), 'new content');

      // Second scan should use cache
      const files2 = await externalContextScanner.scanPaths([tempDir]);
    ...
    it('should respect cache invalidation', async () => {
      // First scan
      await externalContextScanner.scanPaths([tempDir]);
    ...
    it('should invalidate specific path', async () => {
      ...
      try {
        ...
        await externalContextScanner.scanPaths([tempDir, tempDir2]);
