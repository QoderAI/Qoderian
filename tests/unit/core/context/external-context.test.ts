...
describe('externalContext utilities', () => {
  describe('buildExternalContextDisplayEntries', () => {
    it('expands parent segments until every display name is unique', () => {
      const entries = buildExternalContextDisplayEntries([
        '/work/team-a/src/docs',
        '/work/team-b/src/docs',
      ]);
      ...
      expect(new Set(entries.map(entry => entry.displayNameLower)).size).toBe(2);
    });

    it('adds a stable suffix for duplicate normalized paths', () => {
      const entries = buildExternalContextDisplayEntries(['/work/docs', '/work/docs']);

      expect(entries.map(entry => entry.displayName)).toEqual(['work/docs', 'work/docs (2)']);
    });
  });
