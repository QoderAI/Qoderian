...
import { parseClipboardConfig, tryParseClipboardConfig } from '@/qoder/mcp/mcp-config-parser';
import { McpStorage } from '@/qoder/storage/mcp-storage';
...
describe('McpStorage', () => {
  describe('load', () => {
    ...
    it('loads servers with disabledTools from _qoderian metadata', async () => {
      const config = {
        mcpServers: {
      ...
      const adapter = createMockAdapter({
        '.qoder/mcp.json': JSON.stringify(config),
      });
      const storage = new McpStorage(adapter);
      const servers = await storage.load();
    ...
    it('returns empty array when mcpServers is not an object', async () => {
      const adapter = createMockAdapter({
        '.qoder/mcp.json': JSON.stringify({ mcpServers: 'invalid' }),
      });
      const storage = new McpStorage(adapter);
      const servers = await storage.load();
      expect(servers).toEqual([]);
    ...
    it('applies defaults when no _qoderian metadata exists', async () => {
      const config = {
        mcpServers: {
          alpha: { command: 'alpha-cmd' },
        },
    ...
    it('loads description from _qoderian metadata', async () => {
      const config = {
  ...
  describe('save', () => {
    it('saves disabledTools to _qoderian metadata', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.qoder/mcp.json']);
      expect(saved._qoderian.servers.alpha.disabledTools).toEqual(['tool_a', 'tool_b']);
    });
    ...
    it('omits disabledTools from metadata when empty', async () => {
      ...
      const saved = JSON.parse(adapter._store['.qoder/mcp.json']);
      // No _qoderian since all fields are default
      expect(saved._qoderian).toBeUndefined();
    });

    it('preserves existing _qoderian metadata when saving', async () => {
      const existing = {
        mcpServers: {
          alpha: { command: 'alpha-cmd' },
        },
      ...
      const adapter = createMockAdapter({
        '.qoder/mcp.json': JSON.stringify(existing),
      });
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.qoder/mcp.json']);
      expect(saved._qoderian.customField).toBe('should be preserved');
      expect(saved._qoderian.servers.alpha.disabledTools).toEqual(['tool_a']);
    });
    ...
    it('saves description to _qoderian metadata', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.qoder/mcp.json']);
      expect(saved._qoderian.servers.alpha.description).toBe('A test server');
    });

    it('stores enabled=false in _qoderian when different from default', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.qoder/mcp.json']);
      expect(saved._qoderian.servers.alpha.enabled).toBe(false);
    });

    it('stores contextSaving=false in _qoderian when different from default', async () => {
      const adapter = createMockAdapter();
      const storage = new McpStorage(adapter);
      ...
      const saved = JSON.parse(adapter._store['.qoder/mcp.json']);
      expect(saved._qoderian.servers.alpha.contextSaving).toBe(false);
    });

    it('removes _qoderian.servers when all metadata is default', async () => {
      const existing = {
        mcpServers: { alpha: { command: 'cmd' } },
        _qoderian: { servers: { alpha: { enabled: false } } },
      };
      const adapter = createMockAdapter({
        '.qoder/mcp.json': JSON.stringify(existing),
      });
    ...
    it('preserves non-servers _qoderian fields when removing servers', async () => {
      const existing = {
        mcpServers: { alpha: { command: 'cmd' } },
        _qoderian: {
          customField: 'keep',
      ...
      const saved = JSON.parse(adapter._store['.qoder/mcp.json']);
      expect(saved._qoderian).toEqual({ customField: 'keep' });
    });

    it('handles corrupted existing file gracefully', async () => {
      const adapter = createMockAdapter({
        '.qoder/mcp.json': 'not json',
      });
      const storage = new McpStorage(adapter);
  ...
  describe('parseClipboardConfig', () => {
    it('parses full Qoder Code format (mcpServers wrapper)', () => {
