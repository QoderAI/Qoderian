...
import { AgentVaultStorage } from '@/qoder/storage/agent-vault-storage';
  ...
  describe('loadAll', () => {
    ...
    it('skips non-markdown files', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        ...
        '.qoder/agents/readme.txt',
        '.qoder/agents/config.json',
      ]);
    ...
    it('continues loading if one file throws', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.qoder/agents/good.md',
        '.qoder/agents/error.md',
        '.qoder/agents/also-good.md',
      ]);
      mockAdapter.read
        .mockResolvedValueOnce(validAgentMd)
    ...
    it('preserves filePath from disk', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.qoder/agents/custom-filename.md',
      ]);
      mockAdapter.read.mockResolvedValue(validAgentMd);

      const agents = await storage.loadAll();

      expect(agents[0].filePath).toBe('.qoder/agents/custom-filename.md');
    });
...
Be strict.`;
      ...
      mockAdapter.listFiles.mockResolvedValue(['.qoder/agents/strict-agent.md']);
  ...
  describe('delete', () => {
    it('deletes using filePath when available', async () => {
      await storage.delete({
        ...
        prompt: 'Review.',
        source: 'vault',
        filePath: '.qoder/agents/custom-filename.md',
      });

      expect(mockAdapter.delete).toHaveBeenCalledWith('.qoder/agents/custom-filename.md');
    });
    ...
    it('falls back to name-based path when no filePath', async () => {
      ...
      expect(mockAdapter.delete).toHaveBeenCalledWith('.qoder/agents/code-reviewer.md');
    });
    ...
    it('converts absolute filePath to vault-relative', async () => {
      await storage.delete({
        ...
        prompt: 'Review.',
        source: 'vault',
        filePath: '/Users/user/vault/.qoder/agents/code-reviewer.md',
      });
    ...
    it('converts Windows absolute filePath to vault-relative', async () => {
      await storage.delete({
        ...
        prompt: 'Review.',
        source: 'vault',
        filePath: 'C:\\Users\\user\\vault\\.qoder\\agents\\custom-filename.md',
      });
  ...
  describe('load', () => {
    it('reads and parses a single agent file', async () => {
      ...
      const result = await storage.load({
        ...
        prompt: '',
        source: 'vault',
        filePath: '.qoder/agents/code-reviewer.md',
      });

      expect(mockAdapter.read).toHaveBeenCalledWith('.qoder/agents/code-reviewer.md');
    ...
    it('resolves Windows absolute filePath before reading', async () => {
      ...
      expect(mockAdapter.read).toHaveBeenCalledWith('.qoder/agents/custom-filename.md');
    });
  });
  ...
  describe('path normalization', () => {
    it('saves using vault-relative path when filePath is absolute', async () => {
      await storage.save({
        ...
        prompt: 'Do stuff.',
        source: 'vault',
        filePath: '/Users/user/vault/.qoder/agents/my-agent.md',
      });

      expect(mockAdapter.write).toHaveBeenCalledWith(
        '.qoder/agents/my-agent.md',
        expect.any(String)
      );
    });
    ...
    it('preserves vault-relative filePath as-is', async () => {
      await storage.save({
        ...
        prompt: 'Do stuff.',
        source: 'vault',
        filePath: '.qoder/agents/custom-name.md',
      });

      expect(mockAdapter.write).toHaveBeenCalledWith(
        '.qoder/agents/custom-name.md',
        expect.any(String)
      );
    });
    ...
    it('saves using custom filename from Windows absolute path', async () => {
      await storage.save({
        ...
        prompt: 'Do stuff.',
        source: 'vault',
        filePath: 'C:\\Users\\user\\vault\\.qoder\\agents\\custom-name.md',
      });
