import { extractFirstParagraph, parseSlashCommandContent, serializeCommand, serializeSlashCommandMarkdown, validateCommandName, validateSkillName, yamlString } from '@/qoder/commands/slash-command';
...
Review this code: $ARGUMENTS`;
      ...
      expect(parsed.model).toBe('qoder-sonnet-4-5');
...
Prompt`;
      ...
      expect(parsed.model).toBe('qoder-haiku-4-5');
    });
...
describe('serializeSlashCommandMarkdown', () => {
  it('serializes all fields in kebab-case', () => {
    ...
    expect(result).toContain('model: qoder-sonnet-4-5');
...
describe('validateSkillName', () => {
  it('uses the same Qoder slug rules with skill-specific errors', () => {
    expect(validateSkillName('code-review')).toBeNull();
    expect(validateSkillName('CodeReview')).toContain('Skill name');
  });
});
