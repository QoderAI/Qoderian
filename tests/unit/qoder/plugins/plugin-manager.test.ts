...
import { PluginManager } from '@/qoder/plugins/plugin-manager';

const mockFs = fs as jest.Mocked<typeof fs>;

// Create a mock QoderCliSettingsStorage
function createMockQoderCliSettingsStorage(settings: Record<string, unknown> = {}) {
  return {
...
const installedPluginsPath = path.join(homeDir, '.qoder', 'plugins', 'installed_plugins.json');
const globalSettingsPath = path.join(homeDir, '.qoder', 'settings.json');
...
describe('PluginManager', () => {
  ...
  describe('loadPlugins', () => {
    it('returns empty array when no installed_plugins.json exists', async () => {
      ...
      const qoderCliSettings = createMockQoderCliSettingsStorage();
      const manager = new PluginManager(vaultPath, qoderCliSettings);
    ...
    it('project false overrides global true', async () => {
      ...
      const qoderCliSettings = createMockQoderCliSettingsStorage(projectSettings);
      const manager = new PluginManager(vaultPath, qoderCliSettings);
    ...
    it('extracts plugin name from ID correctly', async () => {
      const installedPlugins = {
        version: 2,
        plugins: {
          'feature-dev@qoder-plugins-official': [{
            scope: 'user',
  ...
  describe('togglePlugin', () => {
    it('disables an enabled plugin', async () => {
      ...
      expect(qoderCliSettings.setPluginEnabled).toHaveBeenCalledWith('test-plugin@marketplace', false);
    });
    ...
    it('does nothing when plugin not found', async () => {
      ...
      expect(qoderCliSettings.setPluginEnabled).not.toHaveBeenCalled();
    });
  });
  ...
  describe('enablePlugin', () => {
    it('enables a disabled plugin', async () => {
      ...
      expect(manager.getPlugins()[0].enabled).toBe(true);
      expect(qoderCliSettings.setPluginEnabled).toHaveBeenCalledWith('test-plugin@marketplace', true);
    });
  ...
  describe('non-array plugin entries normalization', () => {
    ...
    it('loads bundled plugins that only carry installPath', async () => {
      const installedPlugins = {
        plugins: {
          'better-harness@qoder-bundler': {
            installPath: '/Users/testuser/.qoder/plugins/cache/qoder-bundler/better-harness',
          },
          'security-scan@qoder-bundler': {
            installPath: '/Users/testuser/.qoder/plugins/cache/qoder-bundler/security-scan',
          },
        },
      };
      ...
      expect(plugins.every((p) => p.scope === 'user')).toBe(true);
      expect(plugins.every((p) => p.enabled)).toBe(true);
      expect(Notice).not.toHaveBeenCalled();
    });

    it('skips entries without installPath', async () => {
      const installedPlugins = {
        plugins: {
          'broken-plugin@marketplace': { version: '1.0.0' },
