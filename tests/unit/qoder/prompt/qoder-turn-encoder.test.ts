...
import { encodeQoderTurn } from '@/qoder/prompt/qoder-turn-encoder';
...
describe('encodeQoderTurn', () => {
  ...
  it('should detect /compact command', () => {
    const result = encodeQoderTurn({ text: '/compact' }, mcpManager);
    expect(result.isCompact).toBe(true);
  });

  it('should detect /compact with arguments', () => {
    const result = encodeQoderTurn({ text: '/compact summarize' }, mcpManager);
    expect(result.isCompact).toBe(true);
  });

  it('should not treat "compact" without slash as compact', () => {
    const result = encodeQoderTurn({ text: 'compact this' }, mcpManager);
  ...
  it('should extract and transform MCP mentions', () => {
    ...
    const result = encodeQoderTurn({ text: '@server-a hello' }, mcpManager);
  ...
  it('should handle request with no optional fields', () => {
    const result = encodeQoderTurn({ text: 'plain message' }, mcpManager);
