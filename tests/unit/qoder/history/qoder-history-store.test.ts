...
import {
  ...
  sdkSessionExists,
} from '@/qoder/history/qoder-history-store';
...
describe('sdkSession', () => {
  ...
  describe('deleteSDKSessionArtifacts', () => {
    it('deletes the transcript and its subagent artifact directory', async () => {
      mockExistsSync.mockReturnValue(true);
      mockFsPromises.unlink.mockResolvedValue(undefined);
    ...
    it('does nothing for an invalid session ID', async () => {
      await deleteSDKSessionArtifacts('/Users/test/vault', '../invalid');
  ...
  describe('parseSDKMessageToChat', () => {
    ...
    it('parses an assistant error without text as a formal error block', () => {
      const sdkMsg: SDKNativeMessage = {
        type: 'assistant',
      ...
      expect(chatMsg).toEqual(expect.objectContaining({
        content: '',
        contentBlocks: [{ type: 'error', content: 'server_error' }],
      }));
    });
  ...
  describe('loadSDKSessionMessages', () => {
    it('reloads duplicate assistant and result errors as one formal error block', async () => {
      ...
      mockFsPromises.readFile.mockResolvedValue([
        ...
        JSON.stringify({
          ...
          error: 'billing_error',
          message: { content: [{ type: 'text', text: readableError }] },
        }),
        JSON.stringify({
          type: 'result',
      ...
      const result = await loadSDKSessionMessages('/Users/test/vault', 'session-error');

      expect(result.messages).toHaveLength(2);
      expect(result.messages[1]).toEqual(expect.objectContaining({
        role: 'assistant',
        content: '',
        contentBlocks: [{ type: 'error', content: readableError, code: 'billing_error' }],
      }));
    });
