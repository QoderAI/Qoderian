...
import {
  ...
  transformSDKMessage,
} from '@/qoder/stream/transform-qoder-message';
...
describe('transformSDKMessage', () => {
  ...
  describe('result messages', () => {
    ...
    it('preserves distinct result errors as separate error events', () => {
      const message = msg({
        type: 'result',
        subtype: 'error_during_execution',
        errors: ['Credit limit reached', 'Request was not completed'],
      });

      const results = [...transformSDKMessage(message)];

      expect(results.filter(result => result.type === 'error')).toEqual([
        { type: 'error', content: 'Credit limit reached' },
        { type: 'error', content: 'Request was not completed' },
      ]);
    });
    ...
    it('matches provider-qualified custom model ids against SDK modelUsage keys', () => {
      ...
      const results = [...transformSDKMessage(message, { intendedModel: 'anthropic/qoder-opus-4-6[1m]' })];
    ...
    it('preserves literal exact matches when provider-qualified entries normalize to the same Qoder id', () => {
      const message = msg({
        type: 'result',
        modelUsage: {
          'eu.anthropic.qoder-opus-4-6[1m]': {
            inputTokens: 1000,
            outputTokens: 300,
            cacheReadInputTokens: 0,
            cacheCreationInputTokens: 0,
          ...
          'us.anthropic.qoder-opus-4-6[1m]': {
            inputTokens: 1000,
            outputTokens: 300,
            cacheReadInputTokens: 0,
            cacheCreationInputTokens: 0,
      ...
      const results = [...transformSDKMessage(message, { intendedModel: 'eu.anthropic.qoder-opus-4-6[1m]' })];
    ...
    it('does not heuristically match different custom model ids', () => {
      ...
      const results = [...transformSDKMessage(message, { intendedModel: 'anthropic/qoder-opus-4-6' })];
