import { buildAgentFromFrontmatter, parseAgentFile, parseModel, parsePermissionMode, parseToolsList } from '@/qoder/agents/agent-storage';
...
describe('parseModel', () => {
  ...
  it('returns a Qoder tier model unchanged', () => {
    expect(parseModel('performance')).toBe('performance');
  });

  it('accepts custom model identifiers', () => {
    expect(parseModel('gateway/custom-model')).toBe('gateway/custom-model');
  });

  it('returns inherit for the inherit value', () => {
    expect(parseModel('inherit')).toBe('inherit');
  });

  it('normalizes inherit case-insensitively', () => {
    expect(parseModel('INHERIT')).toBe('inherit');
  });

  it('trims whitespace', () => {
    expect(parseModel('  efficient  ')).toBe('efficient');
  });

  it('preserves unknown model identifiers for forward compatibility', () => {
    expect(parseModel('future-model')).toBe('future-model');
  });
});
...
describe('buildAgentFromFrontmatter', () => {
  ...
  it('preserves custom model identifiers', () => {
    const result = buildAgentFromFrontmatter(
