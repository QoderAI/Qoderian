...
describe('QoderMessageChannel', () => {
  let channel: QoderMessageChannel;
  let warnings: string[];

  beforeEach(() => {
    warnings = [];
    channel = new QoderMessageChannel((message) => warnings.push(message));
  });
  ...
  describe('error handling', () => {
    it('throws error when enqueueing to closed channel', () => {
      channel.close();
      expect(() => channel.enqueue(createTextUserMessage('test'))).toThrow('QoderMessageChannel is closed');
    });
  });
  ...
  describe('extractTextContent with array content blocks', () => {
    it('should extract and merge text from array-format content during active turn', async () => {
      const ch = new QoderMessageChannel();
