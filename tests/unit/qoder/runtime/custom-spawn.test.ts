import type { SpawnOptions } from '@qoder-ai/qoder-agent-sdk';
import { spawn } from 'child_process';

import * as env from '@/core/env/environment';
import { createCustomSpawnFunction } from '@/qoder/runtime/custom-spawn';
...
describe('createCustomSpawnFunction', () => {
  ...
  it('launches Node-backed script commands through node when SDK passes them directly', () => {
    ...
    const signal = new AbortController().signal;
    spawnFn({
      command: '/npm/node_modules/@qoder-ai/qodercli/cli.js',
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      '/custom/node',
      ['/npm/node_modules/@qoder-ai/qodercli/cli.js', '--output-format', 'stream-json'],
  ...
  it('wraps manually configured Windows .cmd commands through cmd.exe', () => {
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      ...
      ['/d', '/s', '/c', '""C:\\Users\\R&D\\AppData\\Roaming\\npm\\qodercli.cmd" --output-format stream-json"'],
