...
import * as envUtils from '@/core/env/environment';
...
import { buildHistoryRebuildRequest } from '@/qoder/runtime/qoder-conversation-session';
import { QoderMessageChannel } from '@/qoder/runtime/qoder-message-channel';
import { buildQoderPromptWithImages } from '@/qoder/runtime/qoder-user-message-factory';
import * as sessionUtils from '@/qoder/runtime/session-context';
...
describe('QoderChatRuntime', () => {
  ...
  function createRuntimeServices(mcpManager: MockMcpServerManager) {
    return {
      mcpManager,
      pluginManager: (mockPlugin as any).pluginManager,
      agentCatalog: {
        applySessionAgents: (names: string[]) =>
          (mockPlugin as any).agentCatalog?.applySessionAgents(names),
      },
    };
  }
  ...
  beforeEach(() => {
    ...
    service = new QoderChatRuntime(
      mockPlugin as QoderianPlugin,
      createRuntimeServices(mockMcpManager),
    );
  });
  ...
  describe('Persistent Query Management', () => {
    ...
    it('should return false when CLI unavailable after force close', async () => {
      ...
      (mockPlugin.getResolvedQoderCliPath as jest.Mock).mockReturnValue(null);
    ...
    it('should return false when CLI unavailable after config change close', async () => {
      ...
      let cliCallCount = 0;
      (mockPlugin.getResolvedQoderCliPath as jest.Mock).mockImplementation(() => {
        cliCallCount++;
        // First call (for config check) returns valid path
        // Second call (after close, for restart) returns null
        return cliCallCount === 1 ? '/usr/local/bin/qoder' : null;
      });
  ...
  describe('Session Restoration', () => {
    it('should restore session with custom model', () => {
      const customModel = 'qoder-3-opus';
  ...
  describe('buildSDKUserMessage', () => {
    it('should build text-only message', () => {
      const message = (service as any).buildSDKUserMessage('Hello Qoder');
  ...
  describe('buildQoderPromptWithImages', () => {
    it('should return plain string when no images', () => {
      const result = buildQoderPromptWithImages('Hello');
      expect(result).toBe('Hello');
    });

    it('should return plain string when images is undefined', () => {
      const result = buildQoderPromptWithImages('Hello', undefined);
      expect(result).toBe('Hello');
    });
    ...
    it('should return async generator when images are provided', async () => {
      ...
      const result = buildQoderPromptWithImages('Describe', images) as AsyncGenerator<unknown>;
    ...
    it('should omit text when prompt is whitespace with images', async () => {
      ...
      const result = buildQoderPromptWithImages('   ', images) as AsyncGenerator<unknown>;
  ...
  describe('Response Handler Management', () => {
    it('should register and unregister handlers', () => {
      ...
      (service as any).responseRouter.register(handler);
      expect((service as any).responseRouter.handlers).toHaveLength(1);

      (service as any).responseRouter.unregister('test-handler');
      expect((service as any).responseRouter.handlers).toHaveLength(0);
    });

    it('should not fail when unregistering non-existent handler', () => {
      (service as any).responseRouter.unregister('nonexistent');
      expect((service as any).responseRouter.handlers).toHaveLength(0);
    });
    ...
    it('should register multiple handlers', () => {
      ...
      (service as any).responseRouter.register(handler1);
      (service as any).responseRouter.register(handler2);
      expect((service as any).responseRouter.handlers).toHaveLength(2);

      (service as any).responseRouter.unregister('h1');
      expect((service as any).responseRouter.handlers).toHaveLength(1);
      expect((service as any).responseRouter.handlers[0].id).toBe('h2');
    });
  });
  ...
  describe('closePersistentQuery handler notification', () => {
    it('should call onDone on all handlers when not preserving', () => {
      ...
      (service as any).responseRouter.handlers = [handler1, handler2];
    ...
    it('should NOT call onDone when preserving handlers', () => {
      ...
      (service as any).messageChannel = { close: jest.fn() };
      (service as any).queryAbortController = { abort: jest.fn() };
      (service as any).responseRouter.handlers = [handler];
  ...
  describe('createApprovalCallback - per-turn preapproved tools', () => {
    const canUseToolOptions = {
      signal: new AbortController().signal,
      toolUseID: 'test-tool-use-id',
    };

    it('should route tools not in the preapproved list through approval', async () => {
      (service as any).currentPreapprovedTools = ['Read', 'Glob'];
      const callback = jest.fn().mockResolvedValue('allow');
      service.setApprovalCallback(callback);
    ...
    it('should route approval normally when the preapproved list is empty', async () => {
      (service as any).currentPreapprovedTools = [];
      const callback = jest.fn().mockResolvedValue('allow');
      service.setApprovalCallback(callback);
    ...
    it('should allow Skill tool even when not in allowedTools', async () => {
      (service as any).currentPreapprovedTools = ['Read'];
      const callback = jest.fn().mockResolvedValue('allow');
      service.setApprovalCallback(callback);
    ...
    it('should use normal approval when currentPreapprovedTools is null', async () => {
      (service as any).currentPreapprovedTools = null;
      const callback = jest.fn().mockResolvedValue('allow');
      service.setApprovalCallback(callback);
  ...
  describe('routeMessage', () => {
    ...
    it('should route session_init event and capture session', async () => {
      ...
      await (service as any).responseRouter.route(message);
    ...
    it('should flush task_notification completion through auto-turn callback without waiting for a result message', async () => {
      (service as any).responseRouter.handlers = [];
    ...
    it('should suppress history rebuild and clear pendingForkSession on fork session init', async () => {
      ...
      service.syncConversationState({
        sessionId: null,
        qoderState: { forkSource: { sessionId: 'old-session', resumeAt: 'asst-uuid-1' } },
      });
  ...
  describe('applyDynamicUpdates', () => {
    ...
    it('should update model when changed', async () => {
      (mockPlugin as any).settings.model = 'qoder-3-opus';

      await (service as any).applyDynamicUpdates({ model: 'qoder-3-opus' });

      expect(mockPersistentQuery.setModel).toHaveBeenCalled();
    });

    it('should not update model when unchanged', async () => {
      await (service as any).applyDynamicUpdates({ model: 'qoder-3-5-sonnet' });
    ...
    it('should restart when entering YOLO mode', async () => {
    ...
    it('should update permission mode from allow-edits to ask', async () => {
      (mockPlugin as any).settings.permissionMode = 'acceptEdits';
      (service as any).currentConfig = (service as any).buildPersistentQueryConfig(
        '/mock/vault/path',
        '/usr/local/bin/qoder',
        [],
      ...
      (mockPlugin as any).settings.permissionMode = 'default';

      await (service as any).applyDynamicUpdates({});

      expect(mockPersistentQuery.setPermissionMode).toHaveBeenCalledWith('default');
      expect((service as any).currentConfig.permissionMode).toBe('default');
      expect((service as any).currentConfig.sdkPermissionMode).toBe('default');
    });

    it('should update permission mode from ask to allow-edits', async () => {
      (mockPlugin as any).settings.permissionMode = 'default';
      (service as any).currentConfig = (service as any).buildPersistentQueryConfig(
        '/mock/vault/path',
        '/usr/local/bin/qoder',
      ...
      expect((service as any).currentConfig.permissionMode).toBe('acceptEdits');
      expect((service as any).currentConfig.sdkPermissionMode).toBe('acceptEdits');
    });

    it('should apply auto mode dynamically without legacy CLI flags', async () => {
      (mockPlugin as any).settings.permissionMode = 'acceptEdits';
      (service as any).currentConfig = (service as any).buildPersistentQueryConfig(
        '/mock/vault/path',
        '/usr/local/bin/qoder',
      ...
      expect((service as any).currentConfig.permissionMode).toBe('auto');
      expect((service as any).currentConfig.sdkPermissionMode).toBe('auto');
    });

    it('should restart when leaving YOLO even if auto mode is already enabled', async () => {
      (mockPlugin as any).settings.permissionMode = 'yolo';
      (service as any).currentConfig = (service as any).buildPersistentQueryConfig(
        '/mock/vault/path',
        '/usr/local/bin/qoder',
    ...
    it('should restart when MCP servers change', async () => {
      ...
      expect(mockPersistentQuery.setMcpServers).not.toHaveBeenCalled();
      expect((service as any).startPersistentQuery).toHaveBeenCalled();
    });

    it('should not restart when allowRestart is false', async () => {
      // Change something that would trigger restart
      (mockPlugin.getResolvedQoderCliPath as jest.Mock).mockReturnValue('/new/path/to/qoder');
    ...
    it('should silently handle model update error', async () => {
      ...
      await expect((service as any).applyDynamicUpdates({ model: 'qoder-3-opus' })).resolves.toBeUndefined();
    });
  ...
  describe('query() method', () => {
    ...
    it('should yield error when vault path is not available', async () => {
      ...
      const chunks = await collectChunks(queryText('hello'));
    ...
    it('should yield error when CLI path is not available', async () => {
      ...
      expect(chunks).toEqual([{ type: 'error', content: expect.stringContaining('Qoder CLI not found') }]);
    });
    ...
    it('should capture session ID from cold-start response', async () => {
      ...
      await collectChunks(queryText('hello'));
    ...
    it('should use persistent query when available', async () => {
      ...
      startSpy.mockImplementation(async (...args: unknown[]) => {
        ...
        const messageChannel = new QoderMessageChannel();
        (service as any).messageChannel = messageChannel;
        (service as any).persistentQuery = sdkMock.query({ prompt: messageChannel, options: { cwd: vaultPath, pathToQoderCLIExecutable: cliPath } as any });
    ...
    it('should rebuild history context when no session but has history', async () => {
      ...
      const chunks = await collectChunks(queryText('follow up', undefined, history));
    ...
    it('should handle errors in cold-start query', async () => {
      ...
      const chunks = await collectChunks(
        queryText('hello', undefined, undefined, { forceColdStart: true })
      );
  ...
  describe('query() - missing node error', () => {
    ...
    it('should yield error when Node.js is missing', async () => {
      jest.spyOn(envUtils, 'getMissingNodeError').mockReturnValueOnce(
        'Qoder Code CLI requires Node.js, but Node was not found'
      );
    ...
    it('should retry with history on session expired error in cold-start', async () => {
      ...
      jest.spyOn(sdkModule, 'query' as any).mockImplementation((...args: unknown[]) => {
        ...
        if (callCount === 1) {
          ...
          gen.close = jest.fn().mockResolvedValue(undefined);
      ...
      const chunks = await collectChunks(
        queryText('follow up', undefined, history, { forceColdStart: true })
      );
  ...
  describe('routeMessage - agents event', () => {
    it('should apply session agent names from init event', async () => {
      const mockAgentCatalog = { applySessionAgents: jest.fn() };
      (mockPlugin as any).agentCatalog = mockAgentCatalog;
      ...
      expect(mockAgentCatalog.applySessionAgents).toHaveBeenCalledWith(['agent1', 'agent2']);
    });

    it('should not throw when agentCatalog.applySessionAgents fails', async () => {
      const mockAgentCatalog = {
        applySessionAgents: jest.fn().mockImplementation(() => {
      ...
      // Should not throw
      await expect((service as any).responseRouter.route(message)).resolves.toBeUndefined();
    });
  });
  ...
  describe('queryViaPersistent - edge cases', () => {
    it('should fall back to cold-start when persistent query is null', async () => {
      ...
      for await (const chunk of (service as any).queryViaPersistent(
        'test', undefined, '/mock/vault/path', '/usr/local/bin/qoder'
      )) {
        chunks.push(chunk);
      }
    ...
    it('should set allowedTools from query options', async () => {
      ...
      (service as any).persistentQuery = mockPQ;
      const mockChannel = new QoderMessageChannel();
      ...
      const handlers = (service as any).responseRouter.handlers;
      ...
      expect((service as any).currentPreapprovedTools).toEqual(['Read', 'Glob', 'Skill']);
    ...
    it('should fall back to cold-start when consumer is not running', async () => {
      ...
      (service as any).messageChannel = new QoderMessageChannel();
    ...
    it('should fall back when channel is closed during enqueue', async () => {
      ...
      const closedChannel = new QoderMessageChannel();
      closedChannel.close();
  ...
  describe('queryViaSDK - abort signal handling', () => {
    ...
    it('should interrupt response when abort signal is triggered during iteration', async () => {
      ...
      for await (const chunk of (service as any).queryViaSDK(
        'hello', '/mock/vault/path', '/usr/local/bin/qoder', undefined, { forceColdStart: true }
      )) {
        chunks.push(chunk);
      }
  ...
  describe('startResponseConsumer - crash recovery', () => {
    ...
    it('should invalidate session when crash recovery restart fails with session expired', async () => {
      ...
      (service as any).sessionManager.setSessionId('my-session', 'qoder-3-5-sonnet');
  ...
  describe('queryViaSDK - stream content dedup and allowedTools', () => {
    ...
    it('should set allowedTools in cold-start query', async () => {
      ...
      const chunks = await collectChunks(
        queryText('hello', undefined, undefined, {
          forceColdStart: true,
  ...
  describe('applyForkState (via syncConversationState)', () => {
    it('sets pendingForkSession and pendingResumeAt when conversation has forkSource but no sessionId', () => {
      service.syncConversationState({
        sessionId: null,
        qoderState: { forkSource: { sessionId: 'source-session', resumeAt: 'asst-uuid-123' } },
      });
    ...
    it('clears pendingForkSession and pendingResumeAt from previous call', () => {
      ...
      service.syncConversationState({
        sessionId: null,
        qoderState: { forkSource: { sessionId: 'source-1', resumeAt: 'asst-1' } },
      });
      expect((service as any).pendingForkSession).toBe(true);
    ...
    it('treats conversation as not pending when the conversation has a session', () => {
      service.syncConversationState({
      ...
      expect(service.getSessionId()).toBe('sdk-session-xyz');
      expect((service as any).pendingForkSession).toBe(false);
      expect((service as any).pendingResumeAt).toBeUndefined();
    });
  });
  ...
  describe('syncConversationState', () => {
    it('resolves fork state before updating the session', () => {
      ...
      service.syncConversationState({
        sessionId: null,
        qoderState: { forkSource: { sessionId: 'source-session', resumeAt: 'assistant-uuid' } },
      }, ['/external/path']);
