...
import { buildInlineEditPrompt, parseInlineEditResponse } from '@/qoder/prompt/inline-edit';
import type { InlineEditRequest } from '@/qoder/services/qoder-inline-edit-service';
import {
  createReadOnlyHook,
  QoderInlineEditService,
} from '@/qoder/services/qoder-inline-edit-service';
...
describe('QoderInlineEditService', () => {
  let service: QoderInlineEditService;
  let mockPlugin: any;
  ...
  describe('editText', () => {
    ...
    it('should return error when qoder CLI not found', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toContain('Qoder CLI not found');
    });
