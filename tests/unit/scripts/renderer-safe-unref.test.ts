import * as rendererSafeUnrefHelpers from '../../../scripts/renderer-safe-unref.js';
...
describe('rendererSafeUnref helpers', () => {
  it('patches the known unsafe timer .unref() bundle sites', () => {
    ...
    expect(result.appliedPatches).toEqual([
      { name: 'qoder-sdk-process-transport-close', count: 1 },
  ...
  it('patches the latest qoder-sdk async close callback shape', () => {
    ...
    expect(result.appliedPatches).toEqual([
      { name: 'qoder-sdk-process-transport-close-async', count: 1 },
    ]);
    expect(result.contents).toContain('processKillTimer.unref?.();');
