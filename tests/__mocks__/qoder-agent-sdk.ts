...
const mockMessages = [
  ...
  { type: 'assistant', message: { content: [{ type: 'text', text: 'Hello, I am Qoder!' }] } },
...
export function setMockAvailableModels(models: any[]) {
...
function getMessagesForPrompt(): any[] {
  ...
  // Older fixtures used the pre-1.0 shorthand `{ type: 'result' }`.
  // Normalize them at the SDK boundary so production code can enforce the
  // real 1.0 contract and require an explicit successful result subtype.
  const messages = baseMessages.map((message) => (
    message.type === 'result' && message.subtype === undefined
...
export function query({ prompt, options }: { prompt: any; options: Options }): AsyncGenerator<any> & { interrupt: () => Promise<void> } {
  ...
  gen.getAvailableModels = jest.fn().mockImplementation(async () => mockAvailableModels);
  gen.initializationResult = jest.fn().mockImplementation(async () => {
    const init = getMessagesForPrompt().find(
      (message) => message.type === 'system' && message.subtype === 'init',
    );
    if (init) {
      return {
        ...init,
        models: mockInitializationModels ?? mockAvailableModels,
      };
    }
    ...
    await new Promise<void>((_resolve, reject) => {
      ...
      if (!signal || signal.aborted) {
        reject(new Error('Query initialization aborted'));
        return;
      }
