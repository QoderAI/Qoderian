...
import { buildResultErrorMessage } from '@test/helpers/sdk-messages';

import type { ChatRuntimeQueryOptions } from '@/core/runtime/types';
import type { ChatMessage, ImageAttachment } from '@/core/types';
import { getActionDescription, getActionPattern } from '@/qoder/runtime/approval-manager';
import { QoderChatRuntime } from '@/qoder/runtime/qoder-chat-runtime';
import { resolveQoderCliPath } from '@/qoder/runtime/qoder-cli-resolver';
import { buildQoderPromptWithImages } from '@/qoder/runtime/qoder-user-message-factory';
import {
  ...
  truncateToolResult,
} from '@/qoder/runtime/session-context';
import { transformSDKMessage } from '@/qoder/stream/transform-qoder-message';
import { getPathFromToolInput } from '@/qoder/tools/tool-input';
...
function createRuntimeServices(plugin: any, mcpManager: any) {
  return {
    mcpManager,
    pluginManager: plugin.pluginManager,
    agentCatalog: {
      applySessionAgents: (names: string[]) => plugin.agentCatalog?.applySessionAgents(names),
    },
  };
}

function queryText(
  service: QoderChatRuntime,
  text: string,
  ...
  return service.query(turn, conversationHistory, queryOptions);
}
...
describe('QoderChatRuntime', () => {
  ...
  beforeEach(() => {
    ...
    service = new QoderChatRuntime(
      mockPlugin,
      createRuntimeServices(mockPlugin, createMockMcpManager()),
    );
  });
  ...
  describe('findQoderCLI', () => {
    beforeEach(() => {
      mockPlugin.getResolvedQoderCliPath.mockImplementation(() =>
        resolveQoderCliPath(
          undefined,  // Hostname path (not used in tests)
          mockPlugin.settings.qoderCliPath
        )
      );
    });
    ...
    it('should find qodercli in ~/.local/bin/qodercli', async () => {
      const homeDir = os.homedir();
      const expectedPath = path.join(homeDir, '.local', 'bin', 'qodercli');
      ...
      const chunks: any[] = [];
      for await (const chunk of queryText(service, 'hello')) {
        chunks.push(chunk);
      }

      const errorChunk = chunks.find(
        (c) => c.type === 'error' && c.content.includes('Qoder CLI not found')
      );
      expect(errorChunk).toBeUndefined();
    });

    it('should return error when qoder CLI not found', async () => {
      ...
      expect(errorChunk?.content).toContain('Qoder CLI not found');
    });

    it('should use custom CLI path when valid file is specified', async () => {
      const customPath = '/custom/path/to/qoder';
      mockPlugin = createMockPlugin({ qoderCliPath: customPath });
      mockPlugin.getResolvedQoderCliPath.mockImplementation(() =>
        resolveQoderCliPath(
          undefined,  // Hostname path (not used in tests)
          mockPlugin.settings.qoderCliPath
      ...
      service = new QoderChatRuntime(mockPlugin, createRuntimeServices(mockPlugin, createMockMcpManager()));
    ...
    it('should fall back to auto-detection when custom path is a directory', async () => {
      ...
      const homeDir = os.homedir();
      const autoDetectedPath = path.join(homeDir, '.local', 'bin', 'qodercli');
    ...
    it('should fall back to auto-detection when custom path does not exist', async () => {
      const customPath = '/nonexistent/path/qoder';
      mockPlugin = createMockPlugin({ qoderCliPath: customPath });
      mockPlugin.getResolvedQoderCliPath.mockImplementation(() =>
        resolveQoderCliPath(
          undefined,  // Hostname path (not used in tests)
    ...
    it('should fall back to auto-detection when custom path stat fails', async () => {
      ...
      expect(options?.pathToQoderCLIExecutable).toBe(autoDetectedPath);
    });

    it('should reload CLI path after cleanup', async () => {
      const firstPath = '/custom/path/to/qoder-1';
      const secondPath = '/custom/path/to/qoder-2';
      mockPlugin = createMockPlugin({ qoderCliPath: firstPath });
      mockPlugin.getResolvedQoderCliPath.mockImplementation(() =>
        resolveQoderCliPath(
          undefined,  // Hostname path (not used in tests)
          mockPlugin.settings.qoderCliPath
      ...
      for await (const _chunk of queryText(service, 'hello')) {
        // drain
      }

      const firstOptions = getLastOptions();
      expect(firstOptions?.pathToQoderCLIExecutable).toBe(firstPath);

      mockPlugin.settings.qoderCliPath = secondPath;
      service.cleanup();
      ...
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      for await (const _chunk of queryText(service, 'hello again')) {
        // drain
      }

      const secondOptions = getLastOptions();
      expect(secondOptions?.pathToQoderCLIExecutable).toBe(secondPath);
    });
  });
  ...
  describe('transformSDKMessage', () => {
    ...
    it('should transform tool_use from assistant message content', async () => {
      ...
      const chunks: any[] = [];
      for await (const chunk of queryText(service, 'read file')) {
        chunks.push(chunk);
      }
    ...
    it('should transform assistant error messages', async () => {
      ...
      const chunks: any[] = [];
      for await (const chunk of queryText(service, 'do something')) {
        chunks.push(chunk);
      }
    ...
    it('should resume previous session on subsequent queries', async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      for await (const _chunk of queryText(service, 'first')) {
        // drain
      }
      ...
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      for await (const _chunk of queryText(service, 'second', undefined, undefined, { forceColdStart: true })) {
        // drain
      }
    ...
    it('should extract multiple content blocks from assistant message', async () => {
      ...
      const chunks: any[] = [];
      for await (const chunk of queryText(service, 'read foo.txt')) {
        chunks.push(chunk);
      }
  ...
  describe('cancel', () => {
    it('should abort ongoing request', async () => {
      ...
      const queryGenerator = queryText(service, 'hello');
    ...
    it('should call interrupt on underlying stream when aborted', async () => {
      ...
      const generator = queryText(service, 'streaming');
  ...
  // QoderMessageChannel tests moved to tests/unit/core/agent/QoderMessageChannel.test.ts

  describe('persistent query updates', () => {
    it('updates model on the active persistent query', async () => {
      const chunks: any[] = [];
      for await (const chunk of queryText(service, 'hello', undefined, undefined, { model: 'qoder-opus-4-5' })) {
        chunks.push(chunk);
      }

      const response = getLastResponse();
      expect(response?.setModel).toHaveBeenCalledWith('qoder-opus-4-5');
    });
  });
  ...
  describe('persistent query error handling', () => {
    it('yields error from assistant message with error field', async () => {
      ...
      const chunks: any[] = [];
      for await (const chunk of queryText(service, 'trigger error')) {
        chunks.push(chunk);
      }
    ...
    it('yields error from failed result messages', async () => {
      ...
      const chunks: any[] = [];
      for await (const chunk of queryText(service, 'trigger max turns')) {
        chunks.push(chunk);
      }
  ...
  describe('closePersistentQuery with preserveHandlers', () => {
    ...
    it('preserves handlers when preserveHandlers is true', async () => {
      ...
      const handlersBefore = (service as any).responseRouter.handlers?.length ?? 0;

      // Close with preserveHandlers: true
      service.closePersistentQuery('test', { preserveHandlers: true });

      // Handlers should still exist
      const handlersAfter = (service as any).responseRouter.handlers?.length ?? 0;
  ...
  describe('persistent query recovery after close', () => {
    ...
    it('can start new persistent query after closePersistentQuery', async () => {
      ...
      const chunks1: any[] = [];
      for await (const chunk of queryText(service, 'first')) {
        chunks1.push(chunk);
      }
      ...
      const chunks2: any[] = [];
      for await (const chunk of queryText(service, 'second')) {
        chunks2.push(chunk);
      }
    ...
    it('can recover after resetSession closes persistent query', async () => {
      ...
      await (service as any).closingQueryPromise;
      expect((service as any).persistentQuery).toBeNull();
  ...
  // QoderSessionManager tests (resetSession, getSessionId, setSessionId) moved to:
  // tests/unit/core/agent/QoderSessionManager.test.ts
  ...
  describe('query with conversation history', () => {
    ...
    it('should accept optional conversation history parameter', async () => {
      ...
      const chunks: any[] = [];
      for await (const chunk of queryText(service, 'new message', undefined, history)) {
        chunks.push(chunk);
      }
  ...
  describe('session restoration', () => {
    it('should use restored session ID on subsequent queries', async () => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      for await (const _chunk of queryText(service, 'continue')) {
        // drain
      }
  ...
  describe('extended thinking', () => {
    ...
    it('should transform thinking blocks from assistant messages', async () => {
      ...
      const chunks: any[] = [];
      for await (const chunk of queryText(service, 'think about this')) {
        chunks.push(chunk);
      }
    ...
    it('should transform thinking deltas from stream events', async () => {
      ...
      const chunks: any[] = [];
      for await (const chunk of queryText(service, 'think')) {
        chunks.push(chunk);
      }
  ...
  describe('session expiration recovery', () => {
    ...
    it('should include tool call info with input (status only for success)', () => {
      ...
      // Successful tools show input but no result (Qoder can re-read if needed)
    ...
    it('should include error messages for failed tool calls with input', () => {
      ...
      // Failed tools include input AND error message so Qoder knows what went wrong
  ...
  describe('session expiration recovery flow', () => {
    beforeEach(() => {
      (fs.existsSync as jest.Mock).mockReturnValue(true);
      mockPlugin.getResolvedQoderCliPath.mockReturnValue('/mock/qoder');
    });
    ...
    it('should rebuild history and retry without resume on session expiration', async () => {
      ...
      const chunks: any[] = [];
      for await (const chunk of queryText(service, 'Follow up', undefined, history, { forceColdStart: true })) {
        chunks.push(chunk);
      }
    ...
    it('should rebuild history when persistent query throws session expired', async () => {
      ...
      const chunks: any[] = [];
      for await (const chunk of queryText(service, 'Follow up', undefined, history)) {
        chunks.push(chunk);
      }
    ...
    it('should preserve current message images when session expired during cold-start', async () => {
      ...
      const chunks: any[] = [];
      for await (const chunk of queryText(service, 'Follow up with image', currentImages, history, { forceColdStart: true })) {
        chunks.push(chunk);
      }
    ...
    it('should preserve current message images when session expired during persistent query', async () => {
      ...
      const chunks: any[] = [];
      for await (const chunk of queryText(service, 'Follow up with images', currentImages, history)) {
        chunks.push(chunk);
      }
  ...
  describe('image prompt and hydration', () => {
    ...
    it('should return plain prompt when no valid images', () => {
      const prompt = buildQoderPromptWithImages('hello', []);
    ...
    it('should build async generator with image blocks', async () => {
      ...
      const gen = buildQoderPromptWithImages('hi', images) as AsyncGenerator<any>;
      const messages: any[] = [];
  ...
  describe('remaining business branches', () => {
    ...
    it('yields error when session retry also fails', async () => {
      ...
      const chunks: any[] = [];
      for await (const c of queryText(service, 'Hi', undefined, history, { forceColdStart: true })) chunks.push(c);
    ...
    it('yields error for non-session failures', async () => {
      ...
      const chunks: any[] = [];
      for await (const c of queryText(service, 'Hi', undefined, undefined, { forceColdStart: true })) chunks.push(c);
  ...
  describe('persistent query dynamic updates', () => {
    it('restarts without bypass capability when going from YOLO to allow-edits', async () => {
      // Start in YOLO mode
      ...
      // Switch to the SDK's allow-edits mode
      mockPlugin.settings.permissionMode = 'acceptEdits';

      const chunks2: any[] = [];
      for await (const c of queryText(service, 'second')) chunks2.push(c);

      const response = getLastResponse();
      expect(getQueryCallCount()).toBe(queryCountBefore + 1);
      expect(response?.setPermissionMode).not.toHaveBeenCalled();
      expect(getLastOptions()?.permissionMode).toBe('acceptEdits');
      expect(getLastOptions()?.allowDangerouslySkipPermissions).toBeUndefined();
    });

    it('restarts with bypass capability when going from allow-edits to YOLO', async () => {
      // Start in allow-edits mode
      mockPlugin.settings.permissionMode = 'acceptEdits';
      service = new QoderChatRuntime(mockPlugin, createRuntimeServices(mockPlugin, createMockMcpManager()));
      ...
      expect(getQueryCallCount()).toBe(queryCountBefore + 1);
      expect(response?.setPermissionMode).not.toHaveBeenCalled();
      expect(getLastOptions()?.permissionMode).toBe('bypassPermissions');
      expect(getLastOptions()?.allowDangerouslySkipPermissions).toBe(true);
    });
    ...
    it('restarts the persistent query when the active MCP server set changes', async () => {
      const mcpManager = createMockMcpManager();
      mcpManager.getActiveServers.mockImplementation((mentions: Set<string>) =>
        Object.fromEntries(
          [...mentions].map((name) => [name, { command: name, args: [] }]),
        ),
      );
      service = new QoderChatRuntime(mockPlugin, createRuntimeServices(mockPlugin, mcpManager));

      const chunks1: any[] = [];
      for await (const c of queryText(service, 'first', undefined, undefined, {
        mcpMentions: new Set(['server1']),
      })) chunks1.push(c);

      const response1 = getLastResponse();
      expect(response1?.setMcpServers).not.toHaveBeenCalled();
      const queryCountBefore = getQueryCallCount();

      // Query with different MCP mentions
      const chunks2: any[] = [];
      for await (const c of queryText(service, 'second', undefined, undefined, {
        mcpMentions: new Set(['server2']),
      })) chunks2.push(c);

      const response2 = getLastResponse();
      expect(response2?.setMcpServers).not.toHaveBeenCalled();
      expect(getQueryCallCount()).toBeGreaterThan(queryCountBefore);
    });
    ...
    it('falls back to cold-start when restart fails during dynamic updates', async () => {
      ...
      mockPlugin.getResolvedQoderCliPath.mockReset();
      mockPlugin.getResolvedQoderCliPath
        .mockReturnValueOnce('/mock/qoder')
        .mockReturnValueOnce('/mock/qoder')
  ...
  describe('persistent query crash recovery', () => {
    ...
    it('clears lastSentMessage on successful completion', async () => {
      ...
      // Run a query
      const chunks: any[] = [];
      for await (const c of queryText(service, 'test')) chunks.push(c);
  ...
  describe('persistent query crash recovery behavior', () => {
    it('restarts persistent query after consumer error to prepare for next query', async () => {
      ...
      const chunks: any[] = [];
      for await (const c of queryText(service, 'initial')) chunks.push(c);
    ...
    it('re-enqueues pending message after crash recovery restart', async () => {
      ...
      try {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        for await (const _chunk of queryText(service, 'initial')) {
          // drain
        }
