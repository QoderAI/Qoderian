...
import { extractMcpMentions } from '@/qoder/mcp/mcp-mentions';
import { McpServerManager } from '@/qoder/mcp/mcp-server-manager';
import { testMcpServer } from '@/qoder/mcp/mcp-tester';
import { MCP_CONFIG_PATH, McpStorage } from '@/qoder/storage/mcp-storage';
...
describe('McpStorage', () => {
  describe('parseClipboardConfig', () => {
    it('should parse full Qoder Code format', () => {
  ...
  describe('load/save', () => {
    it('should preserve unknown top-level keys and merge _qoderian', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
      ...
      expect(saved._qoderian).toEqual({
        extra: { keep: true },
        servers: {
          'new-server': {
            enabled: false,
    ...
    it('should keep existing _qoderian fields when metadata is defaulted', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
      ...
      const saved = JSON.parse(files.get(MCP_CONFIG_PATH) || '{}') as Record<string, unknown>;
      expect(saved._qoderian).toEqual({ extra: { keep: true } });
    ...
    it('should remove _qoderian when only servers metadata exists', async () => {
      const initial = {
        mcpServers: {
          legacy: { command: 'node' },
        },
