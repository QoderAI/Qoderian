...
const CLASS_DISPLAY: Record<string, string> = {
  ...
  'qoderian-tab-content': 'flex',
  'qoderian-thinking-effort': 'flex',
};

const DISPLAY_CLASSES = new Set([
  'qoderian-hidden',
  'qoderian-visible-block',
  'qoderian-visible-flex',
...
export function createMockEl(tag = 'div'): any {
  ...
  const resolveDisplay = (): string | null => {
    if (classes.has('qoderian-hidden')) return 'none';
    if (classes.has('qoderian-visible-flex')) return 'flex';
    if (classes.has('qoderian-visible-block')) return 'block';
