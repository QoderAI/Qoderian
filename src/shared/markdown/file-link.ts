...
import { getVaultFileByPath } from '../obsidian/compat';
...
function repairEmptyInternalLink(app: App, link: HTMLAnchorElement): void {
  ...
  link.classList.add('qoderian-file-link');
 ...
 * Handles both our custom .qoderian-file-link and Obsidian's .internal-link.
 */
export function registerFileLinkHandler(
  ...
  component.registerDomEvent(container, 'click', (event: MouseEvent) => {
    ...
    const link = target.closest('.qoderian-file-link, .internal-link') as HTMLAnchorElement;
  ...
  const walker = container.ownerDocument.createTreeWalker(
    ...
    NodeFilter.SHOW_TEXT,
    {
      acceptNode(node) {
        ...
        if (parent.closest('pre, code, a, .qoderian-file-link, .internal-link')) {
          return NodeFilter.FILTER_REJECT;
        }
