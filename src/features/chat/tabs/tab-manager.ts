...
import { chooseForkTarget } from '../ui/fork-target-modal';
import {
  activateTab,
  createTab,
  deactivateTab,
...
export class TabManager implements TabManagerInterface {
  ...
  async createTab(
    ...
    // Initialize UI components with the Qoder command catalog.
    initializeTabUI(tab, this.plugin, {
      getQoderCatalogConfig: () => this.getQoderCatalogConfig(tab),
    });
  ...
  invalidateQoderCommandCaches(): void {
    this.plugin.qoderServices.commandCatalog.invalidateRuntimeCommands();
    for (const tab of this.tabs.values()) {
      tab.ui?.slashCommandDropdown?.resetCatalogCache();
    }
  }
  ...
  private async createForkConversation(context: ForkContext): Promise<string> {
    ...
    const forkQoderState = this.plugin.qoderServices.historyService.buildForkQoderState(
      context.sourceSessionId,
      context.resumeAt,
      context.sourceQoderState,
    );

    await this.plugin.updateConversation(conversation.id, {
      messages: context.messages,
      qoderState: forkQoderState,
      ...(title && { title }),
  ...
  // ============================================
  // Qoder Command Catalog
  // ============================================

  private getQoderCatalogConfig(_tab: TabData) {
    const catalog = this.plugin.qoderServices.commandCatalog;
