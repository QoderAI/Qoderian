/**
 * Qoderian - Chat instruction modal
 *
...
export class InstructionModal extends Modal {
  ...
  onOpen() {
    ...
    contentEl.addClass('qoderian-instruction-modal');
    this.setTitle('Add custom instruction');

    // User input section (always visible)
    const inputSection = contentEl.createDiv({ cls: 'qoderian-instruction-section' });
    const inputLabel = inputSection.createDiv({ cls: 'qoderian-instruction-label' });
    inputLabel.setText('Your input:');
    const inputText = inputSection.createDiv({ cls: 'qoderian-instruction-original' });
    inputText.setText(this.rawInstruction);

    // Main content section (changes based on state)
    this.contentSectionEl = contentEl.createDiv({ cls: 'qoderian-instruction-content-section' });

    // Loading state
    this.loadingEl = this.contentSectionEl.createDiv({ cls: 'qoderian-instruction-loading' });
    this.loadingEl.createDiv({ cls: 'qoderian-instruction-spinner' });
    this.loadingEl.createSpan({ text: 'Processing your instruction...' });

    // Clarification state (hidden initially)
    this.clarificationEl = this.contentSectionEl.createDiv({ cls: 'qoderian-instruction-clarification-section' });
    this.clarificationEl.addClass('qoderian-hidden');
    this.clarificationTextEl = this.clarificationEl.createDiv({ cls: 'qoderian-instruction-clarification' });

    const responseSection = this.clarificationEl.createDiv({ cls: 'qoderian-instruction-section' });
    const responseLabel = responseSection.createDiv({ cls: 'qoderian-instruction-label' });
    responseLabel.setText('Your response:');

    this.responseTextarea = new TextAreaComponent(responseSection);
    this.responseTextarea.inputEl.addClass('qoderian-instruction-response-textarea');
    ...
    this.confirmationEl = this.contentSectionEl.createDiv({ cls: 'qoderian-instruction-confirmation-section' });
    this.confirmationEl.addClass('qoderian-hidden');

    // Refined instruction display/edit
    const refinedSection = this.confirmationEl.createDiv({ cls: 'qoderian-instruction-section' });
    const refinedLabel = refinedSection.createDiv({ cls: 'qoderian-instruction-label' });
    refinedLabel.setText('Refined snippet:');

    this.refinedDisplayEl = refinedSection.createDiv({ cls: 'qoderian-instruction-refined' });
    this.editContainerEl = refinedSection.createDiv({ cls: 'qoderian-instruction-edit-container' });
    this.editContainerEl.addClass('qoderian-hidden');

    this.editTextarea = new TextAreaComponent(this.editContainerEl);
    this.editTextarea.inputEl.addClass('qoderian-instruction-edit-textarea');
    this.editTextarea.inputEl.rows = 4;

    // Buttons (changes based on state)
    this.buttonsEl = contentEl.createDiv({ cls: 'qoderian-instruction-buttons' });
    this.updateButtons();
  ...
  showClarificationLoading() {
    ...
    if (this.loadingEl) {
      this.loadingEl.querySelector('.qoderian-instruction-spinner');
  ...
  private showState(state: ModalState) {
    ...
    if (this.loadingEl) {
      this.loadingEl.toggleClass('qoderian-hidden', state !== 'loading');
    }
    if (this.clarificationEl) {
      this.clarificationEl.toggleClass('qoderian-hidden', state !== 'clarification');
    }
    if (this.confirmationEl) {
      this.confirmationEl.toggleClass('qoderian-hidden', state !== 'confirmation');
    }
  ...
  private updateButtons() {
    ...
    const cancelBtn = this.buttonsEl.createEl('button', {
      text: 'Cancel',
      cls: 'qoderian-instruction-btn qoderian-instruction-reject-btn',
    ...
    if (this.state === 'clarification') {
      const submitBtn = this.buttonsEl.createEl('button', {
        text: 'Submit',
        cls: 'qoderian-instruction-btn qoderian-instruction-accept-btn',
      ...
      this.editBtnEl = this.buttonsEl.createEl('button', {
        text: 'Edit',
        cls: 'qoderian-instruction-btn qoderian-instruction-edit-btn',
  ...
  private toggleEdit() {
    ...
    if (this.isEditing) {
      this.refinedDisplayEl?.addClass('qoderian-hidden');
      this.editContainerEl?.removeClass('qoderian-hidden');
      ...
      if (this.refinedDisplayEl) {
        ...
        this.refinedDisplayEl.removeClass('qoderian-hidden');
      }
      this.editContainerEl?.addClass('qoderian-hidden');
