...
export class FileChipsView {
  ...
  constructor(containerEl: HTMLElement, callbacks: FileChipsViewCallbacks) {
    ...
    this.fileIndicatorEl = this.containerEl.createDiv({ cls: 'qoderian-file-indicator' });
    if (firstChild) {
  ...
  renderCurrentNote(filePath: string | null): void {
    ...
    if (!filePath) {
      this.fileIndicatorEl.removeClass('qoderian-visible-flex');
      this.fileIndicatorEl.addClass('qoderian-hidden');
      return;
    }

    this.fileIndicatorEl.addClass('qoderian-visible-flex');
    this.fileIndicatorEl.removeClass('qoderian-hidden');
  ...
  private renderFileChip(filePath: string, onRemove: () => void): void {
    const chipEl = this.fileIndicatorEl.createDiv({ cls: 'qoderian-file-chip' });

    const iconEl = chipEl.createSpan({ cls: 'qoderian-file-chip-icon' });
    setIcon(iconEl, 'file-text');

    const normalizedPath = filePath.replace(/\\/g, '/');
    const filename = normalizedPath.split('/').pop() || filePath;
    const nameEl = chipEl.createSpan({ cls: 'qoderian-file-chip-name' });
    nameEl.setText(filename);
    nameEl.setAttribute('title', filePath);

    const removeEl = chipEl.createSpan({ cls: 'qoderian-file-chip-remove' });
    removeEl.setText('\u00D7');
    removeEl.setAttribute('aria-label', 'Remove');

    chipEl.addEventListener('click', (e) => {
      if (!(e.target as HTMLElement).closest('.qoderian-file-chip-remove')) {
