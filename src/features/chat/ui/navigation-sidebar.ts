...
export class NavigationSidebar {
  ...
  constructor(
    ...
    this.container = this.parentEl.createDiv({ cls: 'qoderian-nav-sidebar' });

    // Create buttons
    this.topBtn = this.createButton('qoderian-nav-btn-top', 'chevrons-up', 'Scroll to top');
    this.prevBtn = this.createButton('qoderian-nav-btn-prev', 'chevron-up', 'Previous message');
    this.tocBtn = this.createButton('qoderian-nav-btn-toc', 'list-tree', 'Conversation directory');
    this.nextBtn = this.createButton('qoderian-nav-btn-next', 'chevron-down', 'Next message');
    this.bottomBtn = this.createButton('qoderian-nav-btn-bottom', 'chevrons-down', 'Scroll to bottom');
  ...
  private createButton(cls: string, icon: string, label: string): HTMLElement {
    const btn = this.container.createDiv({ cls: `qoderian-nav-btn ${cls}` });
    setIcon(btn, icon);
  ...
  private applyVisibility(): void {
    ...
    this.tocBtn.classList.remove('qoderian-hidden');
  ...
  private getDirectoryEntries(): Array<{ el: HTMLElement; title: string }> {
    return Array.from(this.messagesEl.querySelectorAll<HTMLElement>('.qoderian-message-user, [data-role="user"]'))
      .map(el => ({
        el,
  ...
  private getDirectoryTitle(el: HTMLElement): string {
    ...
    const contentEl = el.querySelector<HTMLElement>('.qoderian-message-content');
  ...
  private nodeContainsDirectoryMessage(node: Node): boolean {
    ...
    return typeof candidate.querySelector === 'function'
      && candidate.querySelector('.qoderian-message-user, [data-role="user"]') !== null;
  }
  ...
  private isDirectoryMessageElement(node: Node): boolean {
    ...
    if (typeof candidate.matches === 'function') {
      return candidate.matches('.qoderian-message-user, [data-role="user"]');
    }
    return candidate.classList?.contains?.('qoderian-message-user') === true
  ...
  private openDirectory(): void {
    ...
    this.closeDirectory();
    this.tocPopover = this.parentEl.createDiv({ cls: 'qoderian-nav-toc-popover' });
    this.tocPopover.createDiv({ cls: 'qoderian-nav-toc-title', text: 'Conversation directory' });
    const listEl = this.tocPopover.createDiv({ cls: 'qoderian-nav-toc-list' });

    if (entries.length === 0) {
      listEl.createDiv({
        cls: 'qoderian-nav-toc-empty',
  ...
  private scrollToMessage(direction: 'prev' | 'next'): void {
    const messages = Array.from(this.messagesEl.querySelectorAll<HTMLElement>('.qoderian-message-user'));
