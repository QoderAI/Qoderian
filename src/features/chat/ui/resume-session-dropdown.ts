/**
 * Qoderian - Resume session dropdown
 *
...
export class ResumeSessionDropdown {
  ...
  constructor(
    ...
    this.dropdownEl = this.containerEl.createDiv({ cls: 'qoderian-resume-dropdown' });
    this.render();
  ...
  handleKeydown(e: KeyboardEvent): boolean {
    if (e.isComposing || !this.isVisible()) return false;
  ...
  private updateSelection(): void {
    const items = this.dropdownEl.querySelectorAll('.qoderian-resume-item');
  ...
  private render(): void {
    ...
    const header = this.dropdownEl.createDiv({ cls: 'qoderian-resume-header' });
    header.createSpan({ text: 'Resume conversation' });

    if (this.conversations.length === 0) {
      this.dropdownEl.createDiv({ cls: 'qoderian-resume-empty', text: 'No conversations' });
      return;
    }

    const list = this.dropdownEl.createDiv({ cls: 'qoderian-resume-list' });

    for (let i = 0; i < this.conversations.length; i++) {
      const conv = this.conversations[i];
      const isCurrent = conv.id === this.currentConversationId;

      const item = list.createDiv({ cls: 'qoderian-resume-item' });
      if (isCurrent) item.addClass('current');
      if (i === this.selectedIndex) item.addClass('selected');

      const iconEl = item.createDiv({ cls: 'qoderian-resume-item-icon' });
      setIcon(iconEl, isCurrent ? 'message-square-dot' : 'message-square');

      const content = item.createDiv({ cls: 'qoderian-resume-item-content' });
      const titleEl = content.createDiv({ cls: 'qoderian-resume-item-title', text: conv.title });
      titleEl.setAttribute('title', conv.title);
      content.createDiv({
        cls: 'qoderian-resume-item-date',
