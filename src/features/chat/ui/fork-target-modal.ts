...
class ForkTargetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('qoderian-fork-target-modal');

    const list = this.contentEl.createDiv({ cls: 'qoderian-fork-target-list' });
  ...
  private createOption(container: HTMLElement, target: ForkTarget, label: string): void {
    const item = container.createDiv({ cls: 'qoderian-fork-target-option', text: label });
