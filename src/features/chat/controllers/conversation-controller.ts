...
import type { ExternalContextSelector, McpServerSelector } from '../ui/input-toolbar';
...
export class ConversationController {
  ...
  async createNew(options: { force?: boolean } = {}): Promise<void> {
    ...
    try {
      ...
      welcomeEl.createDiv({ cls: 'qoderian-welcome-greeting', text: this.getGreeting() });
  ...
  async rewind(
    ...
    try {
      await this.save(false, {
        ...
        resetSession: !prevAssistantUuid,
      });
    } catch (e) {
  ...
  async save(updateLastResponse = false, options?: SaveOptions): Promise<void> {
    ...
    const { updates: sessionUpdates } = agentService && !options?.resetSession
  ...
  private renderHistoryItems(
    ...
    const dropdownHeader = container.createDiv({ cls: 'qoderian-history-header' });
    dropdownHeader.createSpan({ text: 'Conversations' });

    const list = container.createDiv({ cls: 'qoderian-history-list' });
    const allConversations = plugin.getConversationList();

    if (allConversations.length === 0) {
      list.createDiv({ cls: 'qoderian-history-empty', text: 'No conversations' });
      return;
    }
    ...
    for (const conv of conversations) {
      ...
      const iconEl = item.createDiv({ cls: 'qoderian-history-item-icon' });
      setIcon(iconEl, this.getHistoryItemIcon(openState, isRunning));

      const content = item.createDiv({ cls: 'qoderian-history-item-content' });
      const titleEl = content.createDiv({ cls: 'qoderian-history-item-title', text: conv.title });
      titleEl.setAttribute('title', conv.title);
      content.createDiv({
        cls: 'qoderian-history-item-date',
      ...
      const actions = item.createDiv({ cls: 'qoderian-history-item-actions' });
      ...
      if (conv.titleGenerationStatus === 'pending') {
        ...
        const regenerateBtn = actions.createEl('button', { cls: 'qoderian-action-btn' });
      ...
      if (openState === 'closed' && options.onOpenConversationInNewTab) {
        const openInNewTabBtn = actions.createEl('button', {
          cls: 'qoderian-action-btn qoderian-open-new-tab-btn',
        });
      ...
      const renameBtn = actions.createEl('button', { cls: 'qoderian-action-btn' });
      ...
      const deleteBtn = actions.createEl('button', { cls: 'qoderian-action-btn qoderian-delete-btn' });
  ...
  getGreeting(): string {
    ...
    const getTimeGreetings = (): string[] => {
      if (hour >= 5 && hour < 12) {
        return [personalize('Good morning'), 'Coffee and Qoderian time?'];
  ...
  updateWelcomeVisibility(): void {
    ...
    if (this.deps.state.messages.length === 0) {
      welcomeEl.removeClass('qoderian-hidden');
    } else {
  ...
  async regenerateTitle(conversationId: string): Promise<void> {
    ...
    // Title generation is delegated to the Qoder service.
  ...
  // ============================================
  // History Dropdown Rendering (for QoderianView)
  // ============================================

  /**
   * Renders the history dropdown content to a provided container.
   * Used by QoderianView to render the dropdown with custom selection callback.
   */
