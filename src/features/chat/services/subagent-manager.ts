...
} from '../rendering/subagent-renderer';
import type { PendingToolCall } from '../state/types';

type TaskTerminalStatus = Extract<ToolCallInfo['status'], 'completed' | 'error'>;
...
export interface TaskResultInterpreter {
  ...
  extractFinalResultFromTranscript(content: string): string | null;
}
...
export class SubagentManager {
  ...
  private updateSubagentLabel(
    ...
    if (description) {
      ...
      const labelEl = wrapperEl.querySelector('.qoderian-subagent-label');
      if (labelEl) {
    ...
    if (prompt) {
      info.prompt = prompt;
      const promptEl = wrapperEl.querySelector('.qoderian-subagent-prompt-text');
      if (promptEl) {
  ...
  private extractResultFromOutputJsonl(outputContent: string): string | null {
    const inlineResult = this.taskResultInterpreter.extractFinalResultFromTranscript(outputContent);
    if (inlineResult) {
      return inlineResult;
    }
    ...
    return this.taskResultInterpreter.extractFinalResultFromTranscript(fullOutput);
  }
