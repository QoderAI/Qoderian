...
export class InlinePlanApproval {
  ...
  render(): void {
    ...
    this.rootEl.createDiv({ cls: 'qoderian-plan-inline-title', text: 'Plan complete' });

    const actionsEl = this.rootEl.createDiv({ cls: 'qoderian-ask-list' });

    // 1. Implement
    const implementRow = actionsEl.createDiv({ cls: 'qoderian-ask-item' });
    implementRow.addClass('is-focused');
    implementRow.createSpan({ text: '\u203A', cls: 'qoderian-ask-cursor' });
    implementRow.createSpan({ text: '1. ', cls: 'qoderian-ask-item-num' });
    implementRow.createSpan({ text: 'Implement', cls: 'qoderian-ask-item-label' });
    ...
    const reviseRow = actionsEl.createDiv({ cls: 'qoderian-ask-item qoderian-ask-custom-item' });
    reviseRow.createSpan({ text: '\u00A0', cls: 'qoderian-ask-cursor' });
    reviseRow.createSpan({ text: '2. ', cls: 'qoderian-ask-item-num' });
    ...
    cancelRow.createSpan({ text: '\u00A0', cls: 'qoderian-ask-cursor' });
    cancelRow.createSpan({ text: '3. ', cls: 'qoderian-ask-item-num' });
  ...
  private updateFocus(): void {
    for (let i = 0; i < this.items.length; i++) {
      ...
      if (i === this.focusedIndex) {
        ...
        if (item.hasClass('qoderian-ask-custom-item') && this.isInputFocused) {
          const input = item.querySelector('.qoderian-ask-custom-text') as HTMLInputElement;
          if (input) {
            input.blur();
            this.isInputFocused = false;
