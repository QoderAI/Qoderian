...
import type { RenderContentFn } from './message-renderer';
...
export class InlineExitPlanMode {
  ...
  render(): void {
    this.rootEl = this.containerEl.createDiv({ cls: 'qoderian-plan-approval-inline' });

    const titleEl = this.rootEl.createDiv({ cls: 'qoderian-plan-inline-title' });
    titleEl.setText('Plan complete');

    this.planContent = this.readPlanContent();
    if (this.planContent) {
      const contentEl = this.rootEl.createDiv({ cls: 'qoderian-plan-content-preview' });
      if (this.renderContent) {
        void this.renderContent(contentEl, this.planContent);
      } else {
        contentEl.createDiv({ cls: 'qoderian-plan-content-text', text: this.planContent });
      }
    } else if (this.planReadError) {
      this.rootEl.createDiv({
        cls: 'qoderian-plan-content-preview qoderian-plan-read-error',
    ...
    if (allowedPrompts && Array.isArray(allowedPrompts) && allowedPrompts.length > 0) {
      const permEl = this.rootEl.createDiv({ cls: 'qoderian-plan-permissions' });
      permEl.createDiv({ text: 'Requested permissions:', cls: 'qoderian-plan-permissions-label' });
      const listEl = permEl.createEl('ul', { cls: 'qoderian-plan-permissions-list' });
      for (const perm of allowedPrompts) {
        listEl.createEl('li', { text: perm.prompt });
      }
    }

    const actionsEl = this.rootEl.createDiv({ cls: 'qoderian-ask-list' });

    const newSessionRow = actionsEl.createDiv({ cls: 'qoderian-ask-item' });
    newSessionRow.addClass('is-focused');
    newSessionRow.createSpan({ text: '\u203A', cls: 'qoderian-ask-cursor' });
    newSessionRow.createSpan({ text: '1. ', cls: 'qoderian-ask-item-num' });
    newSessionRow.createSpan({ text: 'Approve (new session)', cls: 'qoderian-ask-item-label' });
    ...
    const approveRow = actionsEl.createDiv({ cls: 'qoderian-ask-item' });
    approveRow.createSpan({ text: '\u00A0', cls: 'qoderian-ask-cursor' });
    approveRow.createSpan({ text: '2. ', cls: 'qoderian-ask-item-num' });
    approveRow.createSpan({ text: 'Approve (current session)', cls: 'qoderian-ask-item-label' });
    ...
    const feedbackRow = actionsEl.createDiv({ cls: 'qoderian-ask-item qoderian-ask-custom-item' });
    feedbackRow.createSpan({ text: '\u00A0', cls: 'qoderian-ask-cursor' });
    feedbackRow.createSpan({ text: '3. ', cls: 'qoderian-ask-item-num' });
    ...
    this.rootEl.createDiv({ text: HINTS_TEXT, cls: 'qoderian-ask-hints' });
  ...
  private updateFocus(): void {
    for (let i = 0; i < this.items.length; i++) {
      ...
      if (i === this.focusedIndex) {
        ...
        if (item.hasClass('qoderian-ask-custom-item')) {
          const input = item.querySelector('.qoderian-ask-custom-text') as HTMLInputElement;
          if (input) {
            input.focus();
