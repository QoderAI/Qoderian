...
export function createThinkingBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'qoderian-thinking-block' });

  // Header (clickable to expand/collapse)
  const header = wrapperEl.createDiv({ cls: 'qoderian-thinking-header' });
  ...
  // Label with timer
  const labelEl = header.createSpan({ cls: 'qoderian-thinking-label' });
  ...
  const contentEl = wrapperEl.createDiv({ cls: 'qoderian-thinking-content' });
...
export function finalizeThinkingBlock(state: ThinkingBlockState): number {
  ...
  const header = state.wrapperEl.querySelector('.qoderian-thinking-header');
  if (header) {
