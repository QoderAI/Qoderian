...
import { tryParseClipboardConfig } from '../../../qoder/mcp/mcp-config-parser';
import { testMcpServer } from '../../../qoder/mcp/mcp-tester';
...
export class McpSettingsManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'qoderian-mcp-header' });
    headerEl.createSpan({ text: 'MCP Servers', cls: 'qoderian-mcp-label' });

    const addContainer = headerEl.createDiv({ cls: 'qoderian-mcp-add-container' });
    ...
    const dropdown = addContainer.createDiv({ cls: 'qoderian-mcp-add-dropdown' });

    const stdioOption = dropdown.createDiv({ cls: 'qoderian-mcp-add-option' });
    setIcon(stdioOption.createSpan({ cls: 'qoderian-mcp-add-option-icon' }), 'terminal');
    ...
    const httpOption = dropdown.createDiv({ cls: 'qoderian-mcp-add-option' });
    setIcon(httpOption.createSpan({ cls: 'qoderian-mcp-add-option-icon' }), 'globe');
    ...
    const importOption = dropdown.createDiv({ cls: 'qoderian-mcp-add-option' });
    setIcon(importOption.createSpan({ cls: 'qoderian-mcp-add-option-icon' }), 'clipboard-paste');
    ...
    if (this.servers.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'qoderian-mcp-empty' });
      emptyEl.setText('No mcp servers configured. Click "add" to add one.');
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'qoderian-mcp-list' });
  ...
  private renderServerItem(listEl: HTMLElement, server: ManagedMcpServer) {
    const itemEl = listEl.createDiv({ cls: 'qoderian-mcp-item' });
    if (!server.enabled) {
      itemEl.addClass('qoderian-mcp-item-disabled');
    }

    const statusEl = itemEl.createDiv({ cls: 'qoderian-mcp-status' });
    statusEl.addClass(
      server.enabled ? 'qoderian-mcp-status-enabled' : 'qoderian-mcp-status-disabled'
    );

    const infoEl = itemEl.createDiv({ cls: 'qoderian-mcp-info' });

    const nameRow = infoEl.createDiv({ cls: 'qoderian-mcp-name-row' });

    const nameEl = nameRow.createSpan({ cls: 'qoderian-mcp-name' });
    nameEl.setText(server.name);

    const serverType = getMcpServerType(server.config);
    const typeEl = nameRow.createSpan({ cls: 'qoderian-mcp-type-badge' });
    typeEl.setText(serverType);

    if (server.contextSaving) {
      const csEl = nameRow.createSpan({ cls: 'qoderian-mcp-context-saving-badge' });
      csEl.setText('@');
      csEl.setAttribute('title', 'Context-saving: mention with @' + server.name + ' to enable');
    }

    const previewEl = infoEl.createDiv({ cls: 'qoderian-mcp-preview' });
    ...
    const actionsEl = itemEl.createDiv({ cls: 'qoderian-mcp-actions' });

    const testBtn = actionsEl.createEl('button', {
      cls: 'qoderian-mcp-action-btn',
    ...
    const deleteBtn = actionsEl.createEl('button', {
      cls: 'qoderian-mcp-action-btn qoderian-mcp-delete-btn',
