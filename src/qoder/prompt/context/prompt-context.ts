/**
 * Qoder prompt context utilities.
 *
 * Encodes and decodes the XML context envelope persisted in Qoder turns.
 */
...
function formatContextFilesLine(files: string[]): string {
  const safeFiles = escapeXmlClosingTag(files.join(', '), 'context_files');
