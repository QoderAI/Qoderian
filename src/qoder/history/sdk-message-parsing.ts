...
import { extractUserDisplayContent } from '../prompt/context/prompt-context';
import { extractDiffData } from '../tools/diff';
import { extractResolvedAnswers, extractResolvedAnswersFromResultText } from '../tools/tool-input';
import { TOOL_ASK_USER_QUESTION } from '../tools/tool-names';
...
export function parseSDKMessageToChat(
  ...
  const hasAssistantError = sdkMsg.type === 'assistant' && !!sdkMsg.error?.trim();
  if (
    !textContent
    && !hasToolUse
    && !hasImages
  ...
  if (sdkMsg.type === 'assistant' && sdkMsg.error?.trim()) {
    new TurnErrorAccumulator(chatMessage).reconcile(chatMessage, {
      content: sdkMsg.error,
    });
  }
...
export function mergeAssistantMessage(target: ChatMessage, source: ChatMessage): void {
  ...
  if (source.contentBlocks) {
    target.contentBlocks = target.contentBlocks || [];
    const errorAccumulator = new TurnErrorAccumulator(target);
    for (const block of source.contentBlocks) {
      if (block.type === 'error') {
        errorAccumulator.reconcile(target, block);
      } else {
