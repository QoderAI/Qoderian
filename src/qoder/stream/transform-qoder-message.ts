import type { SDKMessage, SDKResultError } from '@qoder-ai/qoder-agent-sdk';

import type { StreamChunk, ToolUseResult, UsageInfo } from '../../core/types';
import { getContextWindowSize, isDefaultQoderModel } from '../models/model-catalog';
import { extractToolResultContent } from '../tools/tool-result-content';
import { isBlockedMessage } from './messages';
import { createTransformStreamState, type TransformStreamState } from './tool-input-stream-state';
import type { TransformEvent } from './types';

type ToolUseFields = { id: string; name: string; input: Record<string, unknown> };
type ToolResultFields = { id: string; content: string; isError?: boolean; toolUseResult?: ToolUseResult };
...
function normalizeQoderModelId(model: string): string {
  ...
  return qoderIndex >= 0 ? normalized.slice(qoderIndex) : normalized;
}

function parseQoderModelSignature(model: string): QoderModelSignature | null {
  const normalized = normalizeQoderModelId(model);
  if (normalized === 'ultimate') {
    return { normalizedModel: normalized, family: 'opus', is1M: false };
  }
  if (normalized === 'performance') {
    return { normalizedModel: normalized, family: 'sonnet', is1M: false };
  }
  if (normalized === 'efficient' || normalized === 'lite') {
  ...
  const versionedMatch = normalized.match(
    /^qoder-(haiku|sonnet|opus)-(\d+)(?:-(\d+))?(?:-(\d{8}))?(?:-v\d+:\d+)?(\[1m\])?$/,
  );
  if (versionedMatch) {
    const [, familyMatch, major, minor, date, oneMillionSuffix] = versionedMatch;
    const family = familyMatch as QoderModelSignature['family'];
    return {
...
function matchQoderModelSignature(
  entrySignature: QoderModelSignature | null,
  intendedSignature: QoderModelSignature,
...
function selectContextWindowEntry(
  ...
  const normalizedIntendedModel = normalizeQoderModelId(intendedModel);
  const exactMatch = findUniqueEntry(entries, (entry) => normalizeQoderModelId(entry.model) === normalizedIntendedModel);
  if (exactMatch) {
    return exactMatch;
  }

  if (!isDefaultQoderModel(intendedModel)) {
    return null;
  }

  const intendedSignature = parseQoderModelSignature(intendedModel);
  if (!intendedSignature) {
    return null;
  }

  const strictSignatureMatch = findUniqueEntry(entries, (entry) =>
    matchQoderModelSignature(parseQoderModelSignature(entry.model), intendedSignature),
  );
  ...
  return findUniqueEntry(entries, (entry) =>
    matchQoderModelSignature(parseQoderModelSignature(entry.model), intendedSignature, { ignoreIs1M: true }),
  );
}
  ...
  switch (message.type) {
    ...
    case 'assistant': {
      ...
      if (message.message?.content && Array.isArray(message.message.content)) {
        for (const block of message.message.content) {
          const thinking = getStringField(block, 'thinking');
      ...
      // Keep partial assistant content before the terminal error. A later result.errors
      // chunk can then upgrade this machine code without reordering the partial answer.
      if (message.error) {
    ...
    case 'user': {
      ...
      if (message.tool_use_result !== undefined && message.parent_tool_use_id) {
        const toolUseResult = (message.tool_use_result ?? undefined) as ToolUseResult | undefined;
    ...
    case 'stream_event': {
      ...
      if (parentToolUseId === null && event?.type === 'message_start') {
        ...
        const thinking = getStringField(event.content_block, 'thinking');
        if (parentToolUseId === null && thinking) {
          yield { type: 'thinking', content: thinking };
        }
        ...
        } else if (parentToolUseId === null && delta?.type === 'thinking_delta'
          && typeof delta.thinking === 'string' && delta.thinking) {
          yield { type: 'thinking', content: delta.thinking };
        } else if (parentToolUseId === null && delta?.type === 'text_delta'
