...
/**
 * QoderMessageChannel - Queue-based async iterable for persistent queries.
 *
 * Rules:
 ...
 */
export class QoderMessageChannel implements AsyncIterable<SDKUserMessage> {
  ...
  enqueue(message: SDKUserMessage): void {
    if (this.closed) {
      throw new Error('QoderMessageChannel is closed');
    }
    ...
    if (!this.turnActive) {
      if (this.resolveNext) {
        ...
        if (this.queue.length >= MESSAGE_CHANNEL_CONFIG.MAX_QUEUED_MESSAGES) {
          this.onWarning(`[QoderMessageChannel] Queue full (${MESSAGE_CHANNEL_CONFIG.MAX_QUEUED_MESSAGES}), dropping newest`);
          return;
        }
        if (hasAttachments) {
    ...
    if (hasAttachments) {
      ...
      if (existingIdx >= 0) {
        ...
        this.onWarning('[QoderMessageChannel] Attachment message replaced (only one can be queued)');
      } else {
        this.queue.push({ type: 'attachment', message });
      }
      return;
    ...
    if (existingTextIdx >= 0) {
      ...
      if (mergedContent.length > MESSAGE_CHANNEL_CONFIG.MAX_MERGED_CHARS) {
        this.onWarning(`[QoderMessageChannel] Merged content exceeds ${MESSAGE_CHANNEL_CONFIG.MAX_MERGED_CHARS} chars, dropping newest`);
        return;
      }
