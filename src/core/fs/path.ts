...
function resolveRealPath(p: string): string {
  ...
  try {
    const resolved = realpathFn(p);
    if (typeof resolved === 'string' && resolved.length > 0) {
      return resolved;
    }
  } catch {
    // Resolve the nearest existing ancestor below.
  }
  ...
  for (;;) {
    try {
      if (fs.existsSync(current)) {
        ...
        if (typeof resolvedExisting === 'string' && resolvedExisting.length > 0) {
...
export function resolvePathForComparison(value: string): string {
  ...
  return normalizePathForComparison(resolveRealPath(normalizePathForFilesystem(value)));
}
...
export function isPathWithinDirectory(
  ...
  const directoryReal = resolvePathForComparison(directoryPath);
  ...
  const resolvedCandidate = resolvePathForComparison(absCandidate);
