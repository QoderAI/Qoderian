...
import { parsePathEntries, resolveNvmDefaultBin } from '../fs/path';

const isWindows = process.platform === 'win32';
const PATH_SEPARATOR = isWindows ? ';' : ':';
const NODE_EXECUTABLE = isWindows ? 'node.exe' : 'node';
const DEVICE_SETTINGS_STORAGE_KEY = 'qoderian.deviceSettingsKey';
...
export function getMissingNodeError(cliPath: string, enhancedPath?: string): string | null {
  ...
  return 'Qoder CLI requires Node.js, but Node was not found on PATH. Install Node.js or use the native Qoder CLI binary, then restart Obsidian.';
}
...
// Settings still store the established `cliPathsByHost` key.
