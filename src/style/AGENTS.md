...
├── toolbar/        # model-selector, permission-toggle, external-context, mcp-selector
...
├── settings/       # base (shared .qoderian-sp-* panel layout), env-snippets, slash-settings, mcp-settings, plugin-settings, agent-settings
├── accessibility.css
...
- **Prefix**: Qoderian-owned classes should use the `.qoderian-` prefix; shared Obsidian host selectors and generic state classes may remain unprefixed
- **BEM-lite**: Prefer `.qoderian-{block}`, `.qoderian-{block}-{element}`, `.qoderian-{block}--{modifier}` for Qoderian-owned selectors
